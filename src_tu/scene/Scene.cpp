//cpp
/* SHADOW translation unit -- arm9 / Scene (the ROM's own name for the class is
 * dScene_c; see include/Scene.h reading 4).
 *
 ...
 * therefore ResetHardwareRegisters first and ~Scene last. The rule holds
 ...
 * there is no `tubuild.py create` candidate id for Scene and no tu_map boundary
 ...
 *    function is a Scene member and neither is reached from one.
 *
 ...
 *    (0x0202e78c) sit between Scene::Initialise3dGraphics (0x0202e6c8) and
 *    Scene::ResetHardwareRegisters (0x0202e7d4). They are not stapled on: an
 *    object's contribution to a section is one contiguous run, so if the two
 *    Scene functions that straddle them share an object then so do they. What
 ...
 *        0x02092678  _ZTV5Scene's Itanium preamble (offset-to-top, typeinfo)
 *        0x02092680  _ZTV5Scene, 18 slots -> 0x020926c8
 ...
 *
 *    0x0202ebe4 is inside Scene::ResetHardwareRegisters. Objects contribute to
 ...
 *
 * THE FOUR GraphCallbacks ARE NOT IN THIS TU. _ZN5Scene14GraphCallback0Ev..3Ev
 ...
 *   _ZTV5Scene     .data  0x02092680   (slot array; the C++ object starts
 *                                       8 bytes earlier, at 0x02092678)
 * Compiling Scene's key function here does emit a vtable and RTTI records as a
...
/* ------------------------------------------------------------------------- */
/* ROM ordinal 20 -- Scene::ResetHardwareRegisters, 0x0202e7d4, size 0x4c8    */
/* ------------------------------------------------------------------------- */
 ...
 * A Stage member physically resident in the Scene TU is unusual but is what the
...
/* ------------------------------------------------------------------------- */
/* ROM ordinal 17 -- Scene::Initialise3dGraphics, 0x0202e6c8, size 0x74      */
/* ------------------------------------------------------------------------- */
/* recovered: real C++ static member, shared header.
 *
...
void Scene::Initialise3dGraphics()
{
...
/* ------------------------------------------------------------------------- */
/* ROM ordinal 16 -- Scene::ResetFadersAndSound, 0x0202e66c, size 0x5c       */
/* ------------------------------------------------------------------------- */
...
/* ------------------------------------------------------------------------- */
/* ROM ordinal 15 -- Scene::BeforeInitResources, 0x0202e638, size 0x34       */
/* ------------------------------------------------------------------------- */
...
/* ------------------------------------------------------------------------- */
/* ROM ordinal 14 -- Scene::AfterInitResources, 0x0202e62c, size 0xc         */
/* ------------------------------------------------------------------------- */
...
/* ------------------------------------------------------------------------- */
/* ROM ordinal 13 -- Scene::BeforeCleanupResources, 0x0202e5f0, size 0x3c    */
/* ------------------------------------------------------------------------- */
...
/* ------------------------------------------------------------------------- */
/* ROM ordinal 12 -- Scene::AfterCleanupResources, 0x0202e5d0, size 0x20     */
/* ------------------------------------------------------------------------- */
...
/* ------------------------------------------------------------------------- */
/* ROM ordinal 11 -- Scene::BeforeBehavior, 0x0202e3d4, size 0x1fc           */
/* ------------------------------------------------------------------------- */
 ...
 * `mov r2,#0` and stop matching, so the ROM's own Scene translation unit must
...
/* ------------------------------------------------------------------------- */
/* ROM ordinal 10 -- Scene::AfterBehavior, 0x0202e3c8, size 0xc              */
/* ------------------------------------------------------------------------- */
...
/* ------------------------------------------------------------------------- */
/* ROM ordinal 9 -- Scene::BeforeRender, 0x0202e3a4, size 0x24               */
/* ------------------------------------------------------------------------- */
...
/* ------------------------------------------------------------------------- */
/* ROM ordinal 8 -- Scene::AfterRender, 0x0202e398, size 0xc                 */
/* ------------------------------------------------------------------------- */
...
/* ------------------------------------------------------------------------- */
/* ROM ordinal 7 -- Scene::SetSceneToSpawn, 0x0202e36c, size 0x2c            */
/* ------------------------------------------------------------------------- */
/* recovered: real C++ static member, shared header.
 *
...
int Scene::SetSceneToSpawn(u32 sceneID, u32 param)
{
...
/* ------------------------------------------------------------------------- */
/* ROM ordinal 6 -- Scene::StartSceneFade, 0x0202e348, size 0x24             */
/* ------------------------------------------------------------------------- */
/* recovered: real C++ static member, shared header.
 *
...
/* ------------------------------------------------------------------------- */
/* ROM ordinal 5 -- Scene::PrepareToSpawnBoot, 0x0202e300, size 0x48         */
/* ------------------------------------------------------------------------- */
/* recovered: real C++ static member, shared header.
 *
...
/* ------------------------------------------------------------------------- */
/* ROM ordinal 4 -- Scene::SpawnIfNecessary, 0x0202e26c, size 0x94           */
/* ------------------------------------------------------------------------- */
/* recovered: real C++ static member, shared header.
 *
...
/* ------------------------------------------------------------------------- */
/* ROM ordinal 3 -- Scene::SetFaders, 0x0202e1e0, size 0x8c                  */
/* ------------------------------------------------------------------------- */
/* recovered: real C++ static member, shared header.
 *
 ...
 * which Scene::SetAndStopColorFader confirms by loading a FaderColor into r0
...
/* Two globals holding the same pointer. 0x0209f5bc is the one this function
   ...
   on it -- which is the evidence that it holds a fader and not a Scene. */
extern FaderBrightness *data_0209d4ac;
}

void Scene::SetFaders(FaderBrightness *fader)
{
    if (data_0209f5bc) {
...
/* ------------------------------------------------------------------------- */
/* ROM ordinal 2 -- Scene::SetAndStopColorFader, 0x0202e1b4, size 0x2c       */
/* ------------------------------------------------------------------------- */
/* recovered: real C++ static member, shared header.
 *
...
/*   _ZN5SceneD1Ev  0x0202e140  size 0x30   (complete-object destructor)      */
/*   _ZN5SceneD0Ev  0x0202e170  size 0x44   (deleting destructor)             */
/* ------------------------------------------------------------------------- */
/* recovered: real C++ destructor, DEFINED INLINE IN include/Scene.h.
 *
 ...
 * merely including the header emits nothing at all -- `_ZN5SceneD1Ev is not in
 * the object`. The two uses below force the out-of-line copies into existence,
 * exactly as src/_ZN5SceneD1Ev.cpp and src/_ZN5SceneD0Ev.cpp do one apiece.
 ...
 *
 * WHY NO SCAFFOLD. src/_ZN5SceneD1Ev.cpp and src/_ZN5SceneD0Ev.cpp each carry a
 * never-called forcing function (`p->~Scene();` / `delete p;`) because a TU that
 * merely includes the header emits no out-of-line copy -- their own comments
 * record `_ZN5SceneD1Ev is not in the object`. THAT IS NOT TRUE OF THIS TU, and
 * the difference is measured, not assumed: this file defines Scene's key
 * function (BeforeInitResources, the first non-inline virtual Scene declares),
 * so it emits _ZTV5Scene, whose slots 16 and 17 ARE D1 and D0 -- and taking
