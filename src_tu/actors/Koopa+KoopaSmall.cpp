 ...
 * Koopa is a community name; retail RTTI identifies daNknk_c. The emitted
 * Koopa RTTI/name and destructor order are not accepted for production.
 *
...
/* Scalar bridge to the current out-of-line definition; see the source-form experiment. */
...
/* Factory targets the retail vtable address; emitted Koopa RTTI is still incorrect. */
...
int *daNknk_c_classInit_NOKONOKO(void)
{
    ...
    if (p) {
        ...
        p[0] = (int)&_ZTV5Koopa[2];  /* +8 address point; shadow metadata is not licensed. */
...
/* The destructor below emits D0, D1 and D2. Only D0/D1 have manifest ROM
 * slots. D2 and the emitted metadata remain unlicensed, and the compiler's
 * destructor order does not match the ROM. Per-function diagnostics do not
 * establish whole-TU ownership.
 */
