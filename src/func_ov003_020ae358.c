// NONMATCHING: size-exact, remaining pure regperm (div=34). unlocked/idx r3/r1 vs r4/r2; loop p/row r2/r3 vs r3/r2; pad field temps r2 vs r1.
#include "common.h"
...
void func_ov003_020ae358(char* c)
{
    ...
    if (unlocked) {
        ...
        for (i = 0; i < 3; i++) {
            if (_ZN8SaveData19IsCharacterUnlockedEj((unsigned int)i) != 0) {
                u8* row = &data_020a0de8[data_020a0e40 * 4];
                u16 dx = (u16)(row[2] - *(u8*)(c + i + 0x124) + 0x18);
                if (dx < 0x30) {
                    u16 dy = (u16)(row[3] - *(u8*)(c + i + 0x128) + 0x18);
                    if (dy < 0x2b) {
                        *(u8*)(c + 0x133) = 1;
    ...
    if (keys & 0x20) {
        ...
        *(u16*)(c + 0x106) = (*(u16*)((char*)data_020a0e5a + idx * 4) & 0x20) ? 0x10 : 8;
        if (*(u8*)(c + 0x134) != 0)
            nxt = (u8)(nxt - 1);
    } else if (keys & 0x10) {
        ...
        *(u16*)(c + 0x106) = (*(u16*)((char*)data_020a0e5a + idx * 4) & 0x10) ? 0x10 : 8;
        if (*(u8*)(c + 0x134) != *(u8*)(c + 0x130) - 2)
            nxt = (u8)(nxt + 1);
    }
cmp_cur:
    if (nxt == cur)
