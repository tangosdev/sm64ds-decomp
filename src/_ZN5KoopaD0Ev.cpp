 ...
 * folds the Itanium destructor variants into the one ~Koopa() it
...
extern "C" Koopa *_ZN5KoopaD0Ev(Koopa *thiz)
{
    thiz->Koopa::~Koopa();         /* the D1 body, through the one host symbol */
    Koopa::operator delete(thiz);  /* the class-specific delete D0 ends with */
    return thiz;
}
#else
Koopa::~Koopa()
