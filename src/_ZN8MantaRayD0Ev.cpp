 ...
 * return the object to its heap. Nobody writes that; declaring `~MantaRay()`
 ...
 * folds the Itanium destructor variants into the one ~MantaRay() it
...
extern "C" MantaRay *_ZN8MantaRayD0Ev(MantaRay *thiz)
{
    thiz->MantaRay::~MantaRay();      /* the D1 body, through the one host symbol */
    MantaRay::operator delete(thiz);  /* the class-specific delete D0 ends with */
    return thiz;
}
#else
MantaRay::~MantaRay()
