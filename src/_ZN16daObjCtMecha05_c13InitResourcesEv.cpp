 ...
 * "ambiguous" data symbol declared by eleven OTHER overlays' own
 * symbols.txt but not ov065's, and adding an ov065 entry for it drags this
 * overlay's own computed base address, and every other ov065 symbol's
 * offset, out of range (tools/modules.py derives an overlay's base from its
 * lowest symbol). Left as the same unresolved external the un-migrated file
 * already carried.
 *
 * Consequently this function's `complete` marker is left off, exactly
 * matching its pre-existing state: byte-verified in isolation, not wired
 * into the real link.
 */
extern "C" {
