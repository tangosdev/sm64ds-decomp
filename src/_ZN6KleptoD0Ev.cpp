 ...
 * folds the Itanium destructor variants into the one ~Klepto() it
...
extern "C" Klepto *_ZN6KleptoD0Ev(Klepto *thiz)
{
    thiz->Klepto::~Klepto();        /* the D1 body, through the one host symbol */
    Klepto::operator delete(thiz);  /* the class-specific delete D0 ends with */
    return thiz;
}
#else
Klepto::~Klepto()
