 ...
 * return the object to its heap. Nobody writes that; declaring `~Key()`
 ...
 * folds the Itanium destructor variants into the one ~Key() it
...
extern "C" Key *_ZN3KeyD0Ev(Key *thiz)
{
    thiz->Key::~Key();           /* the D1 body, through the one host symbol */
    Key::operator delete(thiz);  /* the class-specific delete D0 ends with */
    return thiz;
}
#else
Key::~Key()
