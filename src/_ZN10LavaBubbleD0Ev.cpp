 ...
 * folds the Itanium destructor variants into the one ~LavaBubble() it
...
extern "C" LavaBubble *_ZN10LavaBubbleD0Ev(LavaBubble *thiz)
{
    thiz->LavaBubble::~LavaBubble();    /* the D1 body, through the one host symbol */
    LavaBubble::operator delete(thiz);  /* the class-specific delete D0 ends with */
    return thiz;
}
#else
