 ...
 * folds the Itanium destructor variants into the one ~Snowball() it
...
extern "C" Snowball *_ZN8SnowballD0Ev(Snowball *thiz)
{
    thiz->Snowball::~Snowball();      /* the D1 body, through the one host symbol */
    Snowball::operator delete(thiz);  /* the class-specific delete D0 ends with */
    return thiz;
}
#else
Snowball::~Snowball()
