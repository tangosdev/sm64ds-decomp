 ...
 * folds the Itanium destructor variants into the one ~WaterRing() it
...
extern "C" WaterRing *_ZN9WaterRingD0Ev(WaterRing *thiz)
{
    thiz->WaterRing::~WaterRing();     /* the D1 body, through the one host symbol */
    WaterRing::operator delete(thiz);  /* the class-specific delete D0 ends with */
    return thiz;
}
#else
WaterRing::~WaterRing()
