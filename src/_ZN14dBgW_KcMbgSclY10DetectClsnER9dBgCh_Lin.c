typedef int s32;
typedef s32 Fix12i;
typedef struct { s32 x, y, z; } Vector3;
struct dActor_c;

extern char data_020a0d0c[];
extern char data_020a0d60[];
extern char data_020a0d1c[];

extern void func_0203aa74(void* thiz, Vector3* v, Vector3* res);
extern void _ZN9dBgCh_Lin13SetObjAndLineERK7Vector3S2_P8dActor_c(void* line, const Vector3* a, const Vector3* b, struct dActor_c* act);
extern void func_02035394(void* line, void* arg);
extern int _ZN7dBgW_Kc10DetectClsnER9dBgCh_Lin(void* thiz, void* line);
extern void func_0203aa10(void* thiz, const Vector3* v, Vector3* res);
extern void func_020375ec(int* d, int* s);
extern void _ZN5dBgPiaSERKS_(void* dst, const void* src);

int _ZN14dBgW_KcMbgSclY10DetectClsnER9dBgCh_Lin(void* thiz, char* line)
{
    Vector3 a, b, gp;
    func_0203aa74(thiz, (Vector3*)(line + 0x38), &a);
    func_0203aa74(thiz, (Vector3*)(line + 0x54), &b);
    unsigned char saved = *(unsigned char*)(line + 0x50);
    _ZN9dBgCh_Lin13SetObjAndLineERK7Vector3S2_P8dActor_c(data_020a0d0c, &a, &b, 0);
    if (saved != 0)
        *(char*)(data_020a0d0c + 0x50) = 1;
    func_02035394(data_020a0d0c, line);
    int r = _ZN7dBgW_Kc10DetectClsnER9dBgCh_Lin(thiz, data_020a0d0c);
    if (r != 0)
    {
        int n = *(int*)(data_020a0d0c + 0x60);
        func_0203aa10(thiz, (const Vector3*)data_020a0d60, &gp);
        func_020375ec((int*)line, (int*)&gp);
        *(int*)(line + 0x60) = n;
        _ZN5dBgPiaSERKS_(line + 0x10, data_020a0d1c);
        *(char*)(line + 0x50) = 1;
    }
    return r;
}
