//cpp
// NONMATCHING: regperm: the pooled base ACTIVE_MESH_COLLIDERS colors into r1 (`ldr r1;ldr r0,[r1,r4,lsl#2]`) but the ROM uses r2. Occupying r1 (e.g. passing the dead `a` param) forces r2 but changes a call arg; no zero-cost lever found. (div=2)
...
int MeshColliderBase::Enable(Actor* a)
{
    s32 i = 0;
    for (;;) {
        if (ACTIVE_MESH_COLLIDERS[i] == 0) {
            func_020395fc(this);
            func_020393fc(this, i);
            ACTIVE_MESH_COLLIDERS[i] = this;
            return 1;
        }
        i++;
        if (i >= 0x18) break;
