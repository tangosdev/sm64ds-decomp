 ...
 * return the object to its heap. Nobody writes that; declaring `~Swoop()` is
 ...
 * folds the Itanium destructor variants into the one ~Swoop() it
...
extern "C" Swoop *_ZN5SwoopD0Ev(Swoop *thiz)
{
    thiz->Swoop::~Swoop();         /* the D1 body, through the one host symbol */
    Swoop::operator delete(thiz);  /* the class-specific delete D0 ends with */
    return thiz;
}
#else
Swoop::~Swoop()
