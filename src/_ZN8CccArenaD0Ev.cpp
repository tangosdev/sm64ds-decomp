 ...
 * folds the Itanium destructor variants into the one ~CccArena() it
...
extern "C" CccArena *_ZN8CccArenaD0Ev(CccArena *thiz)
{
    thiz->CccArena::~CccArena();      /* the D1 body, through the one host symbol */
    CccArena::operator delete(thiz);  /* the class-specific delete D0 ends with */
    return thiz;
}
#else
CccArena::~CccArena()
