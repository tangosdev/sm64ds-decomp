 ...
 * return the object to its heap. Nobody writes that; declaring `~LakituBro()`
 ...
 * folds the Itanium destructor variants into the one ~LakituBro() it
...
extern "C" LakituBro *_ZN9LakituBroD0Ev(LakituBro *thiz)
{
    thiz->LakituBro::~LakituBro();     /* the D1 body, through the one host symbol */
    LakituBro::operator delete(thiz);  /* the class-specific delete D0 ends with */
    return thiz;
}
#else
LakituBro::~LakituBro()
