//cpp
/* Lucky Stars minigame scene (ov006/dScMgBSC_c), 20 functions, members
 * converted from free functions by hand, each byte-verified.
 *
 * `#pragma opt_strength_reduction off` is FILE-GLOBAL and load-bearing:
 * two members need it, bracketing does not bind (last-wins), and the
 * other eighteen are unchanged by its presence. Source runs REVERSE
 * of ROM (highest address first). Do not reorder.
 *
 * Leftover: 27 func_ov004_* + 14 data_* keep linker names (unnamed in
 ...
 * Leftover: 3 unk_NN slot/field names not evidenced.
 */
 ...
 * mShared, so the argument is the same address either way. */
...
/* Vtable slot 19 per tools/rtti_vtables.py --own dScMgBSC_c; the legacy
   ...
   Body: while the table animation (func_ov006_020c1718 on mShared) is still
 ...
 * -0x40000), the deal animation starts (func_ov006_020c1604 on mShared,
 ...
 * mShared; anything still running returns), then the two halves split:
