 ...
 * return the object to its heap. Nobody writes that; declaring `~RollingIronBall()`
 ...
 * folds the Itanium destructor variants into the one ~RollingIronBall() it
...
extern "C" RollingIronBall *_ZN15RollingIronBallD0Ev(RollingIronBall *thiz)
{
    thiz->RollingIronBall::~RollingIronBall();  /* the D1 body, through the one host symbol */
    RollingIronBall::operator delete(thiz);     /* the class-specific delete D0 ends with */
    return thiz;
}
#else
