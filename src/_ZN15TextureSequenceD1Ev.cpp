 ...
 * come out of `struct TextureSequence : Animation` and `virtual ~TextureSequence()` in the
 * header, so there is nothing left to write.
 *
 * The one member is a BTP_File pointer, which has no destructor, so the ROM's
