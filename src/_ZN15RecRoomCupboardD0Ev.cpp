 ...
 * folds the Itanium destructor variants into the one ~RecRoomCupboard() it
...
extern "C" RecRoomCupboard *_ZN15RecRoomCupboardD0Ev(RecRoomCupboard *thiz)
{
    thiz->RecRoomCupboard::~RecRoomCupboard();  /* the D1 body, through the one host symbol */
    RecRoomCupboard::operator delete(thiz);     /* the class-specific delete D0 ends with */
    return thiz;
}
#else
