 ...
 * folds the Itanium destructor variants into the one ~DonutBlock() it
...
extern "C" DonutBlock *_ZN10DonutBlockD0Ev(DonutBlock *thiz)
{
    thiz->DonutBlock::~DonutBlock();    /* the D1 body, through the one host symbol */
    DonutBlock::operator delete(thiz);  /* the class-specific delete D0 ends with */
    return thiz;
}
#else
