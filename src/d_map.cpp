...
/* Allocation factory formerly labelled _ZN6dMap_cC1Ev. It ignores an
 * incoming `this`, allocates sizeof(dMap_c), and is held in a factory table;
...
{
    dMap_c *minimap = (dMap_c *)_ZN7fBase_cnwEj(sizeof(dMap_c));
    if (minimap) {
        _ZN7fBase_cC2Ev(minimap);
        *(void ***)minimap = _ZTV7dBase_c;
        *(void ***)minimap = _ZTV6dMap_c;
    }
    return minimap;
}
