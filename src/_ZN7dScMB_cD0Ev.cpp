 ...
 * folds the Itanium destructor variants into the one ~dScMB_c() it
...
extern "C" dScMB_c *_ZN7dScMB_cD0Ev(dScMB_c *thiz)
{
    thiz->dScMB_c::~dScMB_c();       /* the D1 body, through the one host symbol */
    dScMB_c::operator delete(thiz);  /* the class-specific delete D0 ends with */
    return thiz;
}
#else
dScMB_c::~dScMB_c()
