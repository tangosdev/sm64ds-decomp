 ...
 * return the object to its heap. Nobody writes that; declaring `~Whomp()`
 ...
 * folds the Itanium destructor variants into the one ~Whomp() it
...
extern "C" Whomp *_ZN5WhompD0Ev(Whomp *thiz)
{
    thiz->Whomp::~Whomp();         /* the D1 body, through the one host symbol */
    Whomp::operator delete(thiz);  /* the class-specific delete D0 ends with */
    return thiz;
}
#else
Whomp::~Whomp()
