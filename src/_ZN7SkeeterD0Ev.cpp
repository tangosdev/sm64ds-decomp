 ...
 * return the object to its heap. Nobody writes that; declaring `~Skeeter()`
 ...
 * folds the Itanium destructor variants into the one ~Skeeter() it
...
extern "C" Skeeter *_ZN7SkeeterD0Ev(Skeeter *thiz)
{
    thiz->Skeeter::~Skeeter();       /* the D1 body, through the one host symbol */
    Skeeter::operator delete(thiz);  /* the class-specific delete D0 ends with */
    return thiz;
}
#else
Skeeter::~Skeeter()
