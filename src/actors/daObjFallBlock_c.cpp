//cpp
/* ov098/daObjFallBlock_c -- the class's translation unit.
 * Originally generated by tools/tubuild.py create, then reconciled by hand and
 * promoted; the manifest is config/tu_manifest.d/ov098/daObjFallBlock_c.json.
 *
 * ENROLLED AND CANONICAL. This file is the source of record for the licensed
 * .text run 0x0213a00c..0x0213a794 -- ten functions the ROM build takes from
 * here, byte-for-byte. Twelve bodies are written below and all twelve verify;
 * the two at the bottom (the destructor pair) are deliberately NOT part of the
 * licensed run and are compiled from here only as duplicates the linker drops.
 * See the comment above them and the manifest's boundary_evidence for why.
 *
 ...
 *
 * Assembled from twelve legacy one-function sources (ROM address order). The
 * ten marked ABSORBED were deleted by the promotion and now live only here;
 * the two marked HELD OUT keep their own files and their delinks.txt entries,
 * because the ROM emits D0 below D1 and no admissible source form does that.
 *   [ 0] 0x02139f70  D0Ev                      HELD OUT (own shard)
 *   [ 1] 0x02139fc8  D1Ev                      HELD OUT (own shard)
 *   [ 2] 0x0213a00c  func_ov098_0213a00c       ABSORBED
 *   [ 3] 0x0213a0a8  func_ov098_0213a0a8       ABSORBED
 *   [ 4] 0x0213a0e8  func_ov098_0213a0e8       ABSORBED
 *   [ 5] 0x0213a148  func_ov098_0213a148       ABSORBED
 *   [ 6] 0x0213a17c  Kill                      ABSORBED  (key function)
 *   [ 7] 0x0213a23c  func_ov098_0213a23c       ABSORBED
 *   [ 8] 0x0213a284  OnHitByMegaChar           ABSORBED
 *   [ 9] 0x0213a2cc  func_ov098_0213a2cc       ABSORBED
 *   [10] 0x0213a314  Render                    ABSORBED
 *   [11] 0x0213a36c  Behavior                  ABSORBED
 */
