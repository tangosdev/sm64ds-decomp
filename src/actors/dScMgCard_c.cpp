 ...
 * prize tier rises with the score (1..4). The base does not declare slots
 * 21-35 yet, so the header keeps this a plain member occupying the slot;
 * it becomes an override the day dScMgBase_c.h declares it. Recovered
 * from the vtable slot identity. */
int dScMgCard_c::OnGroundPounded() {
    int x = mHudScore;
    int v;
    if (x < 5) {
        v = 1;
