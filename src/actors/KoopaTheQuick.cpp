 ...
 * 0x0211975c..0x0211af38, ROM ordinals 0..17, 18 functions, assembled from the
 * 18 one-function legacy sources this promotion deletes.
 * config/tu_manifest.d/ov062/KoopaTheQuick.json names every one of them in its
 * per-function `legacy_source` fields, and that manifest is now the record of
 * the file lineage -- do not re-cite the retired shard paths here, they no
 * longer exist in the tree.
 *
 * ENROLLED. This file is the canonical source for the run above: the overlay's
 * delinks entry claims it `complete`, so the ROM build takes those bytes from
 * this TU and not from any per-function object. Every declared function
 * byte-matches the cartridge and every relocation destination was checked
 * before enrolment; the manifest's `verification` block records the toolchain,
 * flags and comparison used.
 *
 * The trailing daRNk_c_classInit at 0x0211aee0 is the class's factory entry,
 * folded in because it sits at zero gap from the run's last member. That
 * [class run][classInit] shape is the established convention here.
 *
 * STILL UNRECONCILED. Promotion rests on byte verification, NOT on source
 * review: the local shadow declarations below were carried verbatim from the
 * legacy sources and have never been reconciled against real project headers
 * (plan sec 7.3). The manifest's `notes` list the declaration conflicts
 * tubuild.py create left commented out for a human to settle -- this TU has
 * many, several on Vector3 and ModelAnim helpers.
 *
