 ...
 * folds the Itanium destructor variants into the one ~KoopaTheQuick() it
...
extern "C" KoopaTheQuick *_ZN13KoopaTheQuickD0Ev(KoopaTheQuick *thiz)
{
    thiz->KoopaTheQuick::~KoopaTheQuick();  /* the D1 body, through the one host symbol */
    KoopaTheQuick::operator delete(thiz);   /* the class-specific delete D0 ends with */
    return thiz;
}
#else
