 ...
 * folds the Itanium destructor variants into the one ~Chuckya() it
...
extern "C" Chuckya *_ZN7ChuckyaD0Ev(Chuckya *thiz)
{
    thiz->Chuckya::~Chuckya();       /* the D1 body, through the one host symbol */
    Chuckya::operator delete(thiz);  /* the class-specific delete D0 ends with */
    return thiz;
}
#else
Chuckya::~Chuckya()
