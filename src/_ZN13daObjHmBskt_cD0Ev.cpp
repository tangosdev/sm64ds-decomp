 ...
 * its heap. Nobody writes that; declaring `~daObjHmBskt_c()` is enough, because
 ...
 * folds the Itanium destructor variants into the one ~daObjHmBskt_c() it
...
extern "C" daObjHmBskt_c *_ZN13daObjHmBskt_cD0Ev(daObjHmBskt_c *thiz)
{
    thiz->daObjHmBskt_c::~daObjHmBskt_c();  /* the D1 body, through the one host symbol */
    daObjHmBskt_c::operator delete(thiz);   /* the class-specific delete D0 ends with */
    return thiz;
}
#else
