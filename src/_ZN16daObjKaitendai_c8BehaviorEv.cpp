 ...
 * 0x021091d4) differs from Platform's in slots 6, 9, 16 and 17 only, and this
 ...
 * Platform helpers are non-virtual, so an unqualified call is already the
 ...
 *
 * Platform::IsClsnInRangeOnScreen is NOT declared in include/Platform.h: its
...
extern "C" int _ZN8Platform21IsClsnInRangeOnScreenE5Fix12IiES1_(void *c, int a, int b);
...
s32 daObjKaitendai_c::Behavior()
{
    ...
    *p = *p + mPrevAngleZ;
    ...
    if (_ZN8Platform21IsClsnInRangeOnScreenE5Fix12IiES1_(this, 0, 0))
        UpdateClsnPosAndRot();
    return 1;
}
