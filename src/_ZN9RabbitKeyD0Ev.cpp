 ...
 * return the object to its heap. Nobody writes that; declaring `~RabbitKey()`
 ...
 * folds the Itanium destructor variants into the one ~RabbitKey() it
...
extern "C" RabbitKey *_ZN9RabbitKeyD0Ev(RabbitKey *thiz)
{
    thiz->RabbitKey::~RabbitKey();     /* the D1 body, through the one host symbol */
    RabbitKey::operator delete(thiz);  /* the class-specific delete D0 ends with */
    return thiz;
}
#else
RabbitKey::~RabbitKey()
