 ...
 * `MeshColliderBase` and `Platform` structs -- with the raw offsets they
 * served. `this + 0x8e` is Actor::mAngleY, and the `t[0x1e/2]` read off
 * `this + 0x300` is this class's own unk_31e reached the long way round.
 *
...
int DonutBlock::Behavior()
{
    /* `+=`, not `mAngleY = mAngleY + ...`: the compound form CSEs the field
       ...
       is what the ROM does here. That is a per-SITE fact -- the opposite
