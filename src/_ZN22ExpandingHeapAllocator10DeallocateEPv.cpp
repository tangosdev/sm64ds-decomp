 ...
 * name them yet.
 *
 * The Target local is constructed normally. FreeNode is the caller-proven static
 * class helper over the embedded free-list sentinel.
 */
...
int ExpandingHeapAllocator::Deallocate(void *ptr)
{
    ...
    UnlinkNode((struct MemoryNode *)(inner + 8), node);
    return FreeNode((struct MemoryNode *)inner, &target);
}
