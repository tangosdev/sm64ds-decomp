//cpp
/* Actor::HorzAngleToCPlayer() at 0x020109b8, 0x2c bytes.
 *
 * The horizontal angle from this actor to the nearest player.
 *
 * The discarded ClosestPlayer() result is the point, not an oversight: that
 * call is what refills data_0209b458, and the global is read immediately after.
 * Calling it for the side effect and then reading the cache is the shape of
 * every reader in this group -- see include/Actor.h.
 *
 ...
 *
 * The old file reached 0x5c through a shadow `struct Actor { char _pad[0x5c];
...
/* Refilled by ClosestPlayer(); see the proximity note in include/Actor.h.
   Declared Actor* rather than void* so the 0x5c read is a field access. */
extern Actor *data_0209b458;   /* nearest player  */
extern Actor *data_0209b450;   /* farthest player */
}

s16 Actor::HorzAngleToCPlayer()
{
    ClosestPlayer();
    return Vec3_HorzAngle((Vector3 *)&mPosX, (Vector3 *)&data_0209b458->mPosX);
}
