...
`tu_map.py` now **fails closed** (exit 1) if `rtti_vtables.json` is missing, stale, or
built from a different commit's `rtti.json`. `--no-rtti` is the deliberate opt-out and is
**refused under `--check`**. `tubuild.py` inherits this, because it shells out to
`tu_map.py` when the map is absent — so a checkout without the prerequisites now gets an
actionable error instead of a silently wrong map. Before that gate existed, a missing
prerequisite produced a complete, self-consistent, **wrong** map with every gate passing.
