...
MATERIALIZED BASE - if the ROM has add rX,base,#OFF then a zero-offset [rX] access, do NOT stop just because ordinary pointer C folds to [base,#OFF]. First try the u64-mask laundering idiom: *(int *)(((int)base + 0xOFF) & 0xFFFFFFFFFFFFFFFF). For halfword/byte offsets >= 0x100, a plain cast often materializes because the encoding forces it. Passing the sub-base as a pointer/call arg can also materialize it.
KNOWN FLOOR - stop, do not grind only after the relevant cheap levers are tried: pure store-EMISSION order, pure register-coloring swaps, pre-indexed writeback from plain C, or repeated no-improvement ordering residuals. Report the best near-miss and finish.

mwccarm idioms (cheap levers on a byte-diff): register allocation follows source DECLARATION ORDER - permute top-of-block C89 local decls; reuse a var vs a new one to save a reg; plain u32/int over enum for intermediates; keep ternaries as ternaries; mirror the ROM's loop form and branch direction; for an address materialized in a reg (add rX,base,#OFF then load) try void* byte arithmetic, then the u64-mask laundering form above when the first access folds; materialize a bool (int b=(x==k); if(b)) to force movne/moveq/cmp. Start from the sibling scaffold and edit toward the disasm.
