...
If you are within ~6 divergences and stalled, read notes/matching-style.md and notes/codegen-rules.md (condensed rules) before giving up; notes/mwccarm-codegen.md holds the full dated idiom catalogue for anything not in codegen-rules.md.
COLORING SWAPS ARE NOT FLOOR BY DEFAULT (revised 2026-07-26): a pure register permutation usually means YOUR source or its pragma crutch perturbs the allocator - read notes/codegen-rules.md's register-allocation-wall rule, then the dated log notes/mwccarm-codegen.md 6aa (remove the pragmas, respell their schedule effects naturally: volatile VRAM pointers, volatile global + temp, (base+i)[K] address form) and 6ab/6ac (dropped call arguments, launder tree position, C89 top decls re-rank) BEFORE reporting floor. Only stop early on residuals whose nearmiss DB entry carries a floor mark with verified mechanism. EXCEPTION: if the coloring swap is a cyclic r4/r5-style swap between a reused pointer/mask and a value that comes from a virtual or other indirect call inside the loop, that is NOT floor yet - see the block-scoped-temp lever below.
