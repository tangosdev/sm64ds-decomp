...
def chain_ranges():
    ...
    while cls:
        ...
        if not h.exists():
            break
        chain.append((cls, h))
        m = re.search(r"^\s*struct\s+" + re.escape(cls) + r"\s*:\s*(\w+)",
                      h.read_text(errors="replace"), re.M)
        cls = m.group(1) if m else None
    sizes = class_sizes()
    out, lo = [], 0
    for cls, h in reversed(chain):
        hi = base_data_size(cls) or sizes.get(cls, lo)
        if hi > lo:
            out.append((str(h.relative_to(REPO)).replace("\\", "/"), lo, hi))
            lo = sizes.get(cls, hi)
    return out
...
def base_member_offsets(base):
    ...
    sizes = class_sizes()
    for ty, star, nm, arr, off in FIELD.findall(h.read_text(errors="replace")):
        if ty in sizes and not star:
            out.add(int(off, 16))
    return out


BASE_MEMBER_OFFSETS = [set()]
...
def members_from_destructor(cls, sizes):
    ...
    Members the BASE owns are skipped -- they are not this class's.
    """
    out = {}
    for suffix in (".c", ".cpp"):
        ...
        for mangled, off in re.findall(
            ...
            name = m.group(2)[:n]
            if len(name) != n or o in BASE_MEMBER_OFFSETS[0] or name not in sizes:
                continue
...
def patch_source(text, oldmap, names, types=None, itypes=None):
    ...
    for nm, o in sorted(oldmap.items(), key=lambda kv: -len(kv[0])):
        if nm in SWALLOWED or o >= BASE_DSIZE[0] or o not in names:
            continue
        base = names[o]
...
def main():
    ...
    BASE[0] = args.base
    BASE_DSIZE[0] = base_data_size(args.base)
    BASE_MEMBER_OFFSETS[0] = base_member_offsets(args.base)
