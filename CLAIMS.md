...
| coddog large-band batches (size >= 0x280): scattered ov006/ov002/arm9 + Player state siblings (addrs 0x020xxxxx) | Session2 (Claude-assisted) | 2026-07-11 | reserved - parallel worker on the >=0x280 size band, disjoint from session 1's [0x100,0x280). API claims 401, coordinated via row. |
| harvest_wl small-func band (size < 0x100): smallest unmatched funcs spread across modules (addrs 0x020xxxxx) | Session3 (Claude-assisted) | 2026-07-11 | in progress - parallel worker on the <0x100 size band, disjoint from S1 [0x100,0x280) and S2 [0x280,0x800]. API claims 401, coordinated via row. |
