...
| `mgr+0x4670` / `mgr+0x46bc[]` | count and table of the spawned kinoko | written by `src/unnamed/ov006/0211/func_ov006_02115b0c.c`, read by `src/unnamed/ov006/0211/func_ov006_02112190.c` |
...
| --- | --- | --- |
| 0x034 | `mZoneCooldown` | `src/game/actors/cMgSmartball_ball_c/_ZN19cMgSmartball_ball_c12SaveSnapshotEv.cpp` decrements it while >0 and refuses to re-latch `mUpperWallSolid` until it reads <= 0. A cooldown, not a lifetime: `RestoreInitial` zeroes it. |
| 0x038 | `mUpperWallSolid` | Set to 1 by `SaveSnapshot` when the ball is inside x [0xe8000,0xf0000) z [0x78000,0xa8000). Its only reader is `src/unnamed/ov006/0211/func_ov006_021126b4.c`, the out-of-bounds predicate, where a non-zero value makes it report z >= 0x78000 in the x band [0xd8000,0xe0000) as blocked. |
...
| 0x040 | `mExpireTimer` | Armed by `src/unnamed/ov006/0211/func_ov006_02111dcc.c`, which refuses to re-arm it while it is positive. `SaveSnapshot` ages it and, on expiry, clears the base's `mIsActive`; after that both `SaveSnapshot` and `Update` return immediately. |
| 0x0fc | `mCollisionCooldown` | Three frames of deafness after a ball-vs-ball hit. `src/unnamed/ov006/0211/func_ov006_02115830.c`, the collision resolver, returns without touching either velocity while EITHER participant's counter is positive, and sets BOTH to 3 on the way out of either path; `SaveSnapshot` ages it and `RestoreInitial` zeroes it. |
| 0x100 | `mIsWaiting` | 1 = still queued. `RestoreInitial` sets it; `SaveSnapshot` clears it once this ball's index equals `mgr+0x4664` and `mInPlay` is up. While set, `SaveSnapshot` runs `src/unnamed/ov006/0211/func_ov006_021128fc.c` (line up behind the predecessor in `mgr+0x4688`); once clear it runs the physics path `src/unnamed/ov006/0211/func_ov006_02112ad8.c`. |
| 0x104 | `mQueueGap` | `src/unnamed/ov006/0211/func_ov006_021128fc.c` writes it twice, both times as `(this ball's index - mgr[0x4664]) * 0xf`. Nothing reads it back -- the name records the arithmetic, which is unambiguous, not a use. |
| 0x108 | `mFrozenPos0` | Written only by `src/unnamed/ov006/0211/func_ov006_02111d4c.c`, which copies `mCurrent0` here. |
| 0x10c | `mFrozenPos1` | Same function, the z half. |
| 0x110 | `mIsFrozen` | Raised by the same `src/unnamed/ov006/0211/func_ov006_02111d4c.c` (which also sets `state3b`). While set, `SaveSnapshot` forces `mCurrent` back to `mFrozenPos`, zeroes the base's velocity pair and returns before any physics. Only `RestoreInitial` clears it. |
...
| 0x11c | `mStuckFrames` | Incremented by `SaveSnapshot` when the ball is out of bounds or has not moved 0x8000; reset to 0 on a refresh. `src/unnamed/ov006/0211/func_ov006_02111df4.c` reports the ball as needing a rescue once it reaches 0x78 -- 120 frames, two seconds. |
...
| 0x121 | `mInPlay` | `src/game/actors/cMgSmartball_ball_c/_ZN19cMgSmartball_ball_c6UpdateEv.cpp` draws nothing while it is 0; `src/unnamed/ov006/0211/func_ov006_02111df4.c` reports a ball with it clear as finished; `src/unnamed/ov006/0211/func_ov006_021128fc.c` tests it on both this ball and its predecessor; `src/unnamed/ov006/0211/func_ov006_02111e7c.c` raises it. `RestoreInitial` clears it only for slot indices >= 9 on the second board. |
| 0x129 | `mExitGateOpen` | `SaveSnapshot` latches it once the ball has crossed the plane through the corner (0xd8000, -0x80000) by more than the base's `mRadius`. Until then `src/unnamed/ov006/0211/func_ov006_021126b4.c` reports z < -0x80000 as out of bounds, and `src/unnamed/ov006/0211/func_ov006_021122e0.c` gates its bottom-right exit test on it. |
...
| offset | name | evidence |
| --- | --- | --- |
| 0x034 | `mReelOffset[3]` (`.x`,`.y`) | `src/game/actors/cMgSmartball_slot_c/_ZN19cMgSmartball_slot_c6UpdateEv.cpp` adds `[i].x` to `mCurrent0` and `[i].y` to `mCurrent1` to place each sprite. `src/unnamed/ov006/0210/func_ov006_0210fb58.c` adds the speed into `.y` and wraps it at 0x30000 -- three symbols of 0x10000 -- which is what makes `.y` a reel position. |
| 0x04c | `mReelVel[3]` (`.x`,`.y`) | `src/unnamed/ov006/0210/func_ov006_0210fb58.c` ramps `.y` up toward 0x3000 while `mIsSpeedingUp`, down to 0x200/0x800 while `mIsSlowingDown`, and adds it into `mReelOffset[i].y` every frame. `.x` is only ever zeroed. |
| 0x064 | `mReelStopCount[3]` | `src/unnamed/ov006/0210/func_ov006_0210fa6c.c` rolls each entry to a random 3..5 when the stop begins; `src/unnamed/ov006/0210/func_ov006_0210fb58.c` spends one per symbol boundary the reel crosses and, at zero, snaps the reel to the boundary and zeroes its speed. |
| 0x070 | `mReelStopping[3]` | `src/unnamed/ov006/0210/func_ov006_0210fb58.c` sets it when that reel's speed bottoms out at 0x800; `src/unnamed/ov006/0210/func_ov006_0210fa6c.c` clears all three. It is the precondition for a reel actually being allowed to stop. |
| 0x073 | `mIsSpinning` | Raised by `src/unnamed/ov006/0210/func_ov006_0210fb04.c`, cleared by `src/unnamed/ov006/0210/func_ov006_0210fb58.c` once every reel has settled. `src/game/actors/cMgSmartball_slot_c/_ZN19cMgSmartball_slot_c12SaveSnapshotEv.cpp` will not start a spin while it is set. |
| 0x074 | `mIsSpeedingUp` | Raised with `mIsSpinning` by `src/unnamed/ov006/0210/func_ov006_0210fb04.c`; `src/unnamed/ov006/0210/func_ov006_0210fb58.c` clears it once all three reels have reached 0x3000. |
| 0x075 | `mIsSlowingDown` | Raised by `src/unnamed/ov006/0210/func_ov006_0210fa6c.c`; while set, `src/unnamed/ov006/0210/func_ov006_0210fb58.c` brakes the reels and counts `mReelStopCount` down. |
| 0x078 | `mSpinsQueued` | `SaveSnapshot` spends one and starts a spin when `mIsSpinning` is clear and `mSpinCooldown` has drained. `src/unnamed/ov006/0210/func_ov006_0210fb04.c` hands the credit straight back -- `++*(int*)(self + 0x78)` -- if a spin was already running, which is what makes this a count of owed spins rather than a timer. |
| 0x07c | `mSpinCooldown` | Set to 0x3c by `src/unnamed/ov006/0210/func_ov006_0210fb58.c` when a spin finishes, aged by `SaveSnapshot`, and blocks the next spin while positive. |
| 0x080 | `mSpinTimer` | Armed with 0xb4 by `src/unnamed/ov006/0210/func_ov006_0210fb04.c` at the start of a spin; `SaveSnapshot` ages it and calls `src/unnamed/ov006/0210/func_ov006_0210fa6c.c` on it reaching exactly 0. |
| 0x084 | `mSoundHandle` | `src/unnamed/ov006/0210/func_ov006_0210fb58.c` does `*(int*)(c+0x84) = Sound_PlayIfNotActive(*(int*)(c+0x84), 2, 0x1a6, 0)` -- the handle fed back in as its own first argument -- and clears it when the loop should stop. |
...
| 0x06c | `mLineTimer[8]` | `src/unnamed/ov006/0210/func_ov006_0210ef48.c` arms the completed line's entry with 0x21d; `SaveSnapshot` ages each one and `Update` draws that line's banner while it is positive. |
| 0x08c | `mLineScored[8]` | Set alongside the timer by `src/unnamed/ov006/0210/func_ov006_0210ef48.c` and never cleared until `RestoreInitial`. `Update` keeps drawing the banner for such a line after its timer expires while the manager's 0x4660 flag is up -- so it is the persistent "this line was completed", not a second timer. |
| 0x094 | `mFanfareTimer` | Armed with 0xb5 by `src/unnamed/ov006/0210/func_ov006_0210ef48.c` on every line completion. `SaveSnapshot` spends it and replays a jingle once a second (`% 60`); `Update` reads bits 0x3c of it to blink. |
| 0x098 | `mLinesScored` | `src/unnamed/ov006/0210/func_ov006_0210ef48.c` increments it once per line it completes; `SaveSnapshot` switches on it to pick one of four jingles. A count, not a timer -- nothing ever decrements it. |
...
| offset | name | evidence |
| --- | --- | --- |
| 0x034 | `mVariant` | Written exactly twice in the tree, both by the spawner `src/unnamed/ov006/0211/func_ov006_02115b0c.c`: objects built from the first tilemap layer get 0, those from the second get 1. The constructor `src/unnamed/ov006/0211/func_ov006_02111b40.c` leaves 2, which is the value `Update`'s switch has no case for. Never assigned at runtime, so it is a kind, not a state. |
| 0x038 | `mWasHit` | Raised by `src/unnamed/ov006/0211/func_ov006_02112190.c`, the ball-vs-kinoko proximity test, on the object it hit (reached through `mgr+0x46bc`). `SaveSnapshot` consumes it: fires the effect and sound, clears the flag and re-arms `mHitTimer`. |
...
| offset | name | evidence |
| --- | --- | --- |
| 0x040 | `mTriggerCount` | Incremented by `src/unnamed/ov006/0210/func_ov006_0210d8bc.c`, which `cMgSmartball_pakkun_c::SaveSnapshot` calls on this object (reached as `mgr+0x4780`) each time the piranha takes a ball. Three readers agree it is a count and not a flag: this class's `SaveSnapshot` eases the angle only while it is > 0, its `Update` draws the two extra base sprites only while it is <= 1, and `src/unnamed/ov006/0210/func_ov006_0210dbb0.c` -- the wing's collision routine -- runs its body test only while it is <= 1. `func_ov006_0210d8bc` itself branches on the value reaching exactly 1, playing sound 0x1a1 the first time and 0x1a2 after. |
| 0x044 | `mAngleSettled` | Raised by `src/unnamed/ov006/0211/func_ov006_021156f8.c`, the manager's "is the table at rest" check, once `src/unnamed/ov006/0210/func_ov006_0210d898.c` reports this wing done -- which is either `mTriggerCount == 0` or the angle at 0x32 having reached its 0x3000 limit. Its one reader is this class's `SaveSnapshot`, which stops easing the angle while it is set. |

Left `unk_`, deliberately:

| offset | why |
| --- | --- |
| 0x034 | `src/unnamed/ov006/0210/func_ov006_0210d8bc.c` sets it to 0x14 on the call that takes `mTriggerCount` to 1 and to 0 on every later call; `RestoreInitial` zeroes it. Nothing reads it back. The shape suggests a timer; a suggestion is not evidence. |
...
| 0x036 | `mSpinSpeed` | `src/game/actors/cMgSmartball_propeller_c/_ZN24cMgSmartball_propeller_c12SaveSnapshotEv.cpp` steps it by +-8 toward `mSpinSpeedTarget`, clamping on overshoot in either direction, and then ADDS it onto the base's angle at 0x32 every frame -- which is what makes it an angular speed and not an angle. `RestoreInitial` zeroes it. |
...
| offset | name | evidence |
| --- | --- | --- |
| 0x034 | `mRespawnTimer` | Armed with 0x3c -- one second -- by `src/unnamed/ov006/0211/func_ov006_021115cc.c`, which `src/unnamed/ov006/0211/func_ov006_02112ad8.c` (the ball physics) calls on this hole the frame a ball falls in; the same call raises the base's `unk_031` and plays the effect. `SaveSnapshot` ages it and, on the frame it reaches zero, calls `src/unnamed/ov006/0211/func_ov006_02114ec0.c` on the manager -- which walks the ball table and puts the first active ball not yet in play INTO play -- and sets the base's `unk_032`. So the field is the delay between losing a ball down this hole and being served the next one. `Update` treats it as exactly that: while `unk_031` is set it picks one of three warning sprites at the 0x28 and 0x14 thresholds, i.e. the countdown is shown to the player. `RestoreInitial` zeroes it. |
...
| offset | name | evidence |
| --- | --- | --- |
| 0x038 | `mDriveBase` | Captured at the trigger frame from the same table as `mDriveNow`, less however far `mCurrent1` already sat above the 0xa0000 rest height. Every active frame `src/game/actors/cMgSmartball_spring_c/_ZN21cMgSmartball_spring_c12SaveSnapshotEv.cpp` recomputes `mCurrent1 = mDriveNow - mDriveBase + 0xa0000`, so this is the zero point the live reading is measured from. |
...
reaches that slot was read: `src/minigames/dScMgSmartball_c/_ZN16dScMgSmartball_c8BehaviorEv.cpp` and
`src/minigames/dScMgSmartball_c/_ZN16dScMgSmartball_c6RenderEv.cpp` only call its virtuals,
`src/unnamed/ov006/0211/func_ov006_02115b0c.c` and
`src/minigames/dScMgSmartball_c/_ZN16dScMgSmartball_c21AfterCleanupResourcesEj.cpp` only build and free it,
and `src/unnamed/ov006/0211/func_ov006_0211248c.c` only reads its position. None of them touches
...
| 0x02c | `mIndex` | `src/unnamed/ov006/0211/func_ov006_021128fc.c` compares it against `mgr+0x4664` and indexes `mgr+0x4688` with it minus one. |
| 0x030 | `mIsActive` | The constructor sets it to 1. `cMgSmartball_ball_c::SaveSnapshot` and `::Update` return immediately while it is 0, `src/unnamed/ov006/0211/func_ov006_02111dcc.c` refuses to arm an expiry on an object with it clear, and both `cMgSmartball_board_c::SaveSnapshot` and `cMgSmartball_kinoko_c::SaveSnapshot` skip a tracked ball whose byte at 0x30 is 0. |
...
| `cMgSmartball_wing_c` | 0x034, 0x038, 0x03c | All three written by `src/unnamed/ov006/0210/func_ov006_0210d8bc.c` on the call that bumps `mTriggerCount`, all three zeroed by `RestoreInitial`, none read back. |
...
| `cMgSmartball_propeller_c` | 0x038..0x077 | A pad, and unmodelled rather than unread: `src/unnamed/ov006/0211/func_ov006_02110e28.c` read-modify-writes eight `(x, y)` pairs there through its own shadow struct. Naming them is that helper's job, not this header's. |
| `cMgSmartball_wing_c` | 0x045..0x087 | The same shape, with `src/unnamed/ov006/0210/func_ov006_0210d93c.c` as the helper. |
