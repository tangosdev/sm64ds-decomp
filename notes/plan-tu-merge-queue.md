...
> **Measured on branch `fix/1607-restack2` (51aac8515), which was 212 commits behind
> `main`.** Every count below — the TU total, the tier populations, the batch queue — is
> from that tree. `main` has since split `config/tu_manifest.json` into
> `config/tu_manifest.d/`, renamed the collision classes to the cartridge's own names
> (`MeshColliderBase` -> `dBgW`, #1643), and landed ~70 more `src_tu` TUs, two of which
> this queue still lists as candidates. **Re-run the generator chain before trusting a
> number here:** `rtti_extract.py` -> `rtti_vtables.py` -> `tu_map.py`, in that order —
> `tu_map.py` writes a self-consistent WRONG map with exit 0 if its inputs are missing.
> On `main` at ec472ac0b the chain yields 522 TUs, not the 524 quoted below.
>
> The generated data files this document cites under `notes/data/` are deliberately NOT
> committed: they are derivable from the chain above and were stale the moment `main`
> moved. The reasoning, the recipes, and the failure modes are what is worth keeping.
...
python tools/tu_preflight.py --batch B3
```
...
#   iterate on src_tu/actors/Squasher.cpp until clean
