...
## Two records, one join key (2026-08-30)

`config/**/symbols.txt` and `symbols/verified.tsv` disagree about hundreds of
addresses at any given moment, and that is structural, not sloppy. Config is where
every correction lands — the class-naming waves (`Actor → dActor_c`), the D1/D2
...
- Target-selection, annotation, and is-it-done tooling joins on **address**,
  module-qualified for overlays — overlay slots alternate, so a bare address can
  be two unrelated functions (ov002/ov004 both base at `0x020ad3e0`; see the
  `KillByAttack` section above).
...
- Anything that needs both records goes through `tools/symrecords.py`, which
  performs the address join and reports disagreement instead of silently picking
  a side. `tools/test_symrecords.py` pins the short list of files allowed to open
  `verified.tsv` directly; a new reader belongs behind the API, not on the list.

`tools/symrecords.py --report` enumerates the whole divergence surface into
`symbols/record_divergences.tsv`, classifies each row (class-rename wave,
dtor/ctor variant, argument rename, real signature fix, canonical mangling, or
needs-a-human), and `--where 0x<addr>` answers "what does each record call this
address, and what does the build compile there" in one line each. Restamping the
mechanically-explained mirror rows to the config spelling is `--restamp`; it is a
dry run unless `--apply` is passed, and it goes to review as a PR, never applied
as a side effect of something else.

One caution baked into the classifier, worth repeating because it is the #774
lesson in new clothes: compiler-generated destructor bodies are byte-identical
across unrelated classes, so a lone diverging destructor pair is **not** evidence
of a class rename. The tool requires a method row to corroborate the class pair
before it will call a destructor divergence mechanical, and everything it cannot
explain stays unproposed for a human.
...
| `symbols/verified.tsv` | the **import mirror** (address → name, flat, no module column). A row is restamped to the config spelling when a correction happens to touch it, and keeps its import-time spelling when no correction does — so it is neither uniformly current nor uniformly stale, and hundreds of addresses spell a different name in each record at any moment. Superseded spellings live in git history. Never a join key for names — see "Two records, one join key" |
| `symbols/record_divergences.tsv` | the banked enumeration of every disagreement between config and the mirror, regenerated by `tools/symrecords.py --report` |
