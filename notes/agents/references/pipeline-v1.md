# Historical v1 pipeline (reference only)

Archived during the GitHub coordination cutover. Operational instructions below
are superseded by [the current protocol](../PIPELINE.md). Preserve the measurements
as dated evidence; do not launch agents, acquire claims, or select paths from them.
