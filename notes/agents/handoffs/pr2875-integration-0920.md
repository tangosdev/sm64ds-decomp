...
The factory is now simply `return new dScMgMemory2_c;`. The real tracker constructor is declared and the shared-state component initializes through its inline constructor. This preserves the exact96-byte factory and both observed component calls. The prior64-byte direct-new experiment omitted those calls; it remains historical and is not a compiler constraint. All52 functions,13 complete metadata records and licensed emission order match. Two extra unreferenced compiler destructor bodies use the existing exact-symbol deadstrip policy, with no unlicensed output or policy errors.

The final object is27,624 bytes, SHA256 `6ce5f6d7a7c72f31b150be561b563332bf04a7add1af2161fe5c56d8e1a66a96`. It is not raw-identical to the old literal factory: the added unused lifecycle bodies and native base-vtable relocation addend are explicitly audited. Every retained canonical payload/target matches. Original credit is preserved; only Memory's queue row changes to one TU/1531lines/52functions. Metadata remains ROM-gap owned.

All17final gates passed:106/106modules,11208 source functions and all26data claims reproduce; 438 expanded strict function instances across 32 source files have no differences or blind relocations. Thirteen static checks and eligibility/reference checks pass. Full stock ROM SHA256 `d1506e90efae5e2d2cf119926a4ac2a291bd5ca78349d09d5024e1a918c478e8`. Global metadata limitations and nine baseline symbol diagnostics remain visible; this does not claim full-repository source or metadata completion.
