...
- Source branch and previous accepted input SHA: branch
  ...
  `origin/main` `c27d8ea38` (`--no-ff`, no conflicts) as `8be0f4c0c`. There is
...
- Original source base SHA and installed workflow/tool SHA: built on
  ...
  (never rebased). Workflow/tool revision
  `f327f7b6460e157153eb7fc0749dbbe60dd854f1`. On BOTH halves used for the ROM
  reports (`c27d8ea38` and this branch) `tools/romdata_check.py` is blob
...
- Reserved source/header/config surfaces actually touched:
  - `config/arm9/overlays/ov022/symbols.txt`, `ov013/symbols.txt`: coined
    ...
    6 rows) promotions. The row inside the held volcano vtable
    (`data_ov022_02114498`) stays as main has it. NO row was added.
...
- Compiler experiments and measured barriers:
  - Baseline ordering: `linkcheck --baseline` builds the working tree, so a
    ...
    body fixed it.
  - daObj_volcanoCannon_c, the HELD promotion: `tubuild linkcheck` refuses in
    `[2/8] dsd delink` -- `Last symbol '_ZTV21daObj_volcanoCannon_c' in
    section '.data' ... has the range 0x0211447c..0x02114500 but is not
    contained within the file's section range (0x02114434..0x021144f8)`. The
    vtable (storage 0x02114474, 8-byte preamble + 31 slots, ends 0x021144f8)
    is the last `.data` object in ov022; the ROM's 0x021144f8..0x02114500 are
    eight zero bytes of `.data` end padding (`align:32`, the overlay file ends
    at 0x02114500) and the next `symbols.txt` row is the `.bss` at
    0x02114500. dsd sizes an unsized last symbol up to the next row. Every one
    of the 21 landed promoted `.data`/`.rodata` claims ends on an existing
    row; PR #2231 solved this by adding a bounding row (`data_ov022_021144f8`)
    that main never carried, which is refused. The class waits for a tool-side
    answer in its own lane.
...
describes (content `851dc888c` merged with `origin/main` `c27d8ea38` as
`8be0f4c0c`), with the pinned mwccarm 2004/b56 and the extracted retail ROM
...
- Full-ROM build: `python tools/rombuild.py -j16 --no-rom --report-json
  build/head-romF.json --data-json build/head-dataF.json`, exit 0. `module
  ...
  0)`, `ROM-build analysis: PASS`, `ROM data from source: 702 verified, 221
  partial, 4 differ`. The same command at `origin/main` `c27d8ea38` in a
  second wired worktree gives the base report (`106/106`, 23/23 data claims,
  `695 verified, 219 partial, 4 differ`).
- Explicit function/consumer relocation checks:
  ...
  - `python tools/linkcheck.py --name <sym> --c src/actors/<Class>.cpp
    ...
    exit 0 each (`build/F-lc-by-symbol.log`).
  - `python tools/validate_merge.py --base c27d8ea38 --head HEAD
    --base-rom-report ... --head-rom-report build/head-romF.json --out
    build/vmF.json`: exit 0, `status Passed`; byte-verified functions
    ...
    from source: 702 exact, 221 partial, 4 differ` (base 695/219/4). Its
...
- Port/path/reference and other applicable static gates:
  ...
  `check_rename_ledger.py` (2017 mangled/vtable rows agree),
  `check_dead_references.py`, `langmode_audit.py --check
  langmode-baseline.json` (PASS), `tiers_ratchet.py --check` (CONVERTED 2699
  -> 2702, not banked), `queue_audit.py --check` (queue agrees with the tree;
  needs `rtti_extract.py`, `rtti_vtables.py --out build/rtti_vtables.json` and
  `tu_map.py --out build/tu_map.json` regenerated AFTER the last config edit
  or it refuses on staleness), `check_references.py` (run after
  ...
  against merge-base `ecc97391e` and main: `symbols/actor_renames.tsv`
  3512 +34 -13 = 3533, `attribution.json` overrides 3105 +17 (+1 after the
  credit fix), `config/converted-baseline.json` 2706 +12 -12 = 2706; zero
  missing, zero extra in each.
