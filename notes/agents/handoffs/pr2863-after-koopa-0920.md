...
Composer `codex-spiny-after-koopa-composer-0920` owns task
`pr2863-after-koopa-0920`, inheriting `pr2863-composition-0920` and all prior
authors/findings. Only the two new report paths are reserved. The worktree is
...
The pinned compiler (`2004/b56`) freshly reproduces all **34 strict functions**,
...
outputs and reversed destructor ordinal pair `(0, 1)` remain, with zero
compiler-only policy errors. These are retained limitations, not waived gates.
This source remains an unenrolled shadow; no full-ROM, production promotion,
private validator or independent Source-review success is claimed here. The
declaration ratchet excludes `src_tu`, so its empty scope does not validate every
shadow contract.

All `SP2863-01` through `SP2863-06` findings retain their kinds and dispositions.
Remaining matrix, class identity, helper/interface and complete-output work stays
partial under [issue #2870](https://github.com/tangosdev/sm64ds-decomp/issues/2870),
next owner `codex-integrator-0920`.

Publish the exact output through this composer's own receipt. Independent reviewer
`/root/repair2747` must claim the offered VERIFY stage with its own receipt and
judge the final candidate/base. Root owns original-PR publication and merge.
