...
make the code say what the prose said. Bodies read: `InitResources`, `Behavior`, `Render` and `CleanupResources`, then in
one-function files under `src/` and since promoted into `src/actors/BlueCoinSwitch.cpp`.
...
The rename carried into the shadow TU as well as `src/`. At the time that TU built
only under a `tuModules` profile, so a stale spelling there compiled nowhere and no
normal gate would have caught it; `tools/check_src_tu_compiles.py` (72/72) and
`tools/check_src_tu.py` were run after. The TU has since been promoted to
`src/actors/BlueCoinSwitch.cpp` and builds in the ordinary ROM build.
...
The rename carried into the shadow TU, now promoted to
`src/actors/daObjCannonShutter_c.cpp`, along with the same
...
The rename carried into the shadow TU, now promoted to
`src/actors/ArrowSignRight.cpp`, as well as `src/`.
