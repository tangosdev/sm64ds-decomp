...
- Exp1 pgmthr-setanim SUPPORTED, Exp2 pgmthr-dropshadow SUPPORTED,
  Extra1 pgmthr-init-1ac SUPPORTED (all three pin `d_a_pg_mthr.cpp:19` sub-claims).
- Extra2 pgmthr-matrix SUPPORTED (`d_a_pg_mthr.cpp:25`).
- Exp3 bakubaku-klass-ice DISPROVED (`d_a_bakubaku.cpp:13`).
- Exp6 bakubaku-matrix SUPPORTED (`d_a_bakubaku.cpp:20`).
- Extra12 bakubaku-deadstore SUPPORTED (`d_a_bakubaku.cpp:29` dead-store representative).
...
- Extra8 wanwan-factory SUPPORTED (`d_a_wanwan.cpp:29`).
- Extra9 wanwan-matrix SUPPORTED (`d_a_wanwan.cpp:32`).
- Extra13 wanwan-stump DISPROVED (`d_a_wanwan.cpp:20` Stump part).
- Extra10 damky-matrix DISPROVED (`daMky_c.cpp:18` matrix part).
...
- `src/actors/daMky_c.cpp:18` (Player/data/factory part) — Player+8/cap+0xc8 on those classes, data_ov030_* handles, factories in other files (matrix part DISPROVED via Extra10).
...
- `src/game/actors/d_a_pg_mthr.cpp:30` — talk/message/func_0201267c names with callees, held-actor slots on Player.
- `src/game/actors/d_a_wanwan.cpp:20` (Player part) — +0x6fb byte name belongs on Player (Stump part DISPROVED via Extra13).
- `src/game/actors/d_a_wanwan.cpp:24` — Pos() not on this branch's dActor_c, needs shared-header decl (#2513 migration).
- `src/game/actors/d_a_s_cre.cpp:13` — Pos() leaf overlay, dActor_c::Pos() is #2513 not this branch.
- `src/game/actors/d_a_bakubaku.cpp:27` — water height + particle/sound helper names belong with callees.
...
- `src/actors/daMky_c.cpp:12` — func_ov030_* ROM labels, coining C++ names changes emitted symbols not bytes.
- `src/game/actors/d_a_bakubaku.cpp:9` — func_ov032_* ROM labels, C++ methods would miss those labels.
- `src/game/actors/d_a_obj_bk_dossunbar.cpp:12` — extern-C state bodies + unowned PMF records, tables filled by sinit not this TU.
- `src/game/actors/d_a_pg_mthr.cpp:10` — func_ov018_* free spelling is reconstruction choice, original ownership unknown.
- `src/game/actors/d_a_wanwan.cpp:11` — func_ov014_* ROM labels, C++ methods would miss those labels.

### Needs shared-header decl / sinit / layout recovery (4, incl. partials)

- `src/game/actors/d_a_bakubaku.cpp:22` — decl_common.h extern-int handles require shared-header change to type cleanly.
- `src/game/actors/d_a_wanwan.cpp:16` — SharedFilePtr has no recovered layout, decl_common char view required.
- `src/actors/daObjLava_c.cpp:13` — wrapper plants default callback; direct New requires callback wiring, not a spelling.
- `src/game/actors/d_a_pg_mthr.cpp:27` — data_ov018_* handles + decl_common int[] + S14 text-only (packaging + shared-header).
...
- `src/actors/daMky_c.cpp:15` — SetAnim wall ref, no sizes.
- `src/actors/daSanbo_c.cpp:65` (wall part) — Init/DropShadow/Spawn wall ref, no sizes.
- `src/game/actors/d_a_bakubaku.cpp:16` — SetAnim wall ref + dBgCh Undefined link claim (link needs full link, not match.py bytes).
- `src/game/actors/d_a_wanwan.cpp:14` — Init/DropShadow wall ref, no sizes.
- `src/game/actors/d_a_obj_bk_dossunbar.cpp:16` (IsClsn/NewSimple part) — wall ref, no sizes (SetFile pinned via Exp5).
- `src/game/actors/d_a_pg_mthr.cpp:19` (TextureSequence/dBgCh part) — no sizes for TextureSequence; dBgCh Undefined needs full link (SetAnim/Init/DropShadow pinned).
- `src/game/actors/d_a_bakubaku.cpp:29` (gotos/register part) — no func/size predicted; dead-store representative pinned via Extra12.
...
- `src/game/actors/d_a_bakubaku.cpp:25` — data_ov032_* handles + sinit IDs + state-table symbols packaging/sinit.
- `src/game/actors/d_a_wanwan.cpp:26` — data_ov014_* handles + sinit IDs + S14 packaging.
