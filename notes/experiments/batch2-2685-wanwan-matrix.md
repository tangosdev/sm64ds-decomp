# common.h first for 02112788 twelve-word copy (BATCH2-2685-02, PR #2685)

- tested_commit: 019a7cc06 (cleanup-leftover-overlay-batch-2 at experiment time)
- compiler: mwccarm 2004/b56 (tools/mwccarm/2004/b56/mwccarm.exe)
- claim: `src/game/actors/d_a_wanwan.cpp:32-33` keeps
  common.h first because Matrix4x3 has two 0x30-byte spellings and
  02112788 copies twelve uniform words.
- attempted_change (scratch-applied, then reverted):

```diff
-#include "common.h"
-#include "daWanwan_c.h"
+#include "daWanwan_c.h"
+#include "common.h"
```

- command:

```text
.venv/bin/python tools/match.py --c src/game/actors/d_a_wanwan.cpp \
  --func func_ov014_02112788 --addr 0x02112788 --size 0x1c4 \
  --version 2004/b56 --module ov014 --strict-relocs --brief
```

- result: baseline MATCH; variant 999 word(s) differ, size 0x1c4 vs 0x200
  (452 -> 512 bytes). Nested spelling scalarizes as claimed.
- log (variant):

```text
TARGET func_ov014_02112788 @ 0x02112788 size 0x1c4  bytes: f04f2de93cd04de20090a0e1fc18d9e1fe28d9e1f039d9e15b0f89e2bca5fceb5c0099e5c001a0e1900189e5600099e5c001a0e1940189e5640099e5c001a0e1... (452 bytes)
  2004/b56: 999 word(s) differ

--- closest: 2004/b56 (999 differ) ---
  size differs: target 0x1c4 vs candidate 0x200

========================================
MATCHING VERSIONS: none
```

- verdict: CONSTRAINT SUPPORTED
