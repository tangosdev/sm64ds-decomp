...
| Physical production source files | 10976 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 10976 |
| Tracked `.c` files | 6194 |
| Tracked `.cpp` files | 4782 |
| `.cpp` files missing first-line `//cpp` | 0 |
| Mangled-symbol source files | 3683 |
| Genuinely migrated C++ symbol files | 3609 |
...
| Source-built (`complete`) source files | 10749 |
| Complete sources owning more than one function | 37 |
| Source-built records inside multi-function sources | 380 |
...
| Measure | Live value |
| --- | ---: |
| Tracked `src_tu/` source files | 62 |
| Manifest entries | 98 |
...
| Entries actually production-enrolled at `promoted_source` | 32 |
| Existing promotion paths that disagree with delinks | 4 |

Manifest statuses: `link-verified` 3, `promoted` 36, `text-verified` 59.
