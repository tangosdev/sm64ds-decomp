...
| Physical production source files | 8930 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 8930 |
| Tracked `.c` files | 5152 |
| Tracked `.cpp` files | 3778 |
...
| Source-built (`complete`) source files | 8772 |
| Complete sources owning more than one function | 142 |
| Source-built records inside multi-function sources | 2574 |
...
| Functions named by manifest entries | 3215 |
| Unique functions named by the manifest | 3204 |
| Modules represented | 50 |
| Manifest shadow sources present in git | 177 |
| Entries licensing non-text sections | 29 |
| Entries actually production-enrolled at `promoted_source` | 146 |
| Existing promotion paths that disagree with delinks | 0 |

Manifest statuses: `link-verified` 3, `promoted` 141, `text-verified` 33.
