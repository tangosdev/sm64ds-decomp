...
| Physical production source files | 10759 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 10759 |
| Tracked `.c` files | 6138 |
| Tracked `.cpp` files | 4621 |
...
| Source-built (`complete`) source files | 10532 |
| Complete sources owning more than one function | 69 |
| Source-built records inside multi-function sources | 630 |
...
| Functions named by manifest entries | 1287 |
| Unique functions named by the manifest | 1287 |
| Modules represented | 30 |
| Manifest shadow sources present in git | 102 |
| Entries licensing non-text sections | 13 |
| Entries actually production-enrolled at `promoted_source` | 64 |
| Existing promotion paths that disagree with delinks | 4 |

Manifest statuses: `link-verified` 3, `promoted` 68, `text-verified` 31.
