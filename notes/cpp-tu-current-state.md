...
| Physical production source files | 9089 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 9089 |
| Tracked `.c` files | 5214 |
| Tracked `.cpp` files | 3875 |
...
| Source-built (`complete`) source files | 8931 |
| Complete sources owning more than one function | 137 |
| Source-built records inside multi-function sources | 2410 |
...
| Functions named by manifest entries | 3051 |
| Unique functions named by the manifest | 3040 |
| Modules represented | 47 |
| Manifest shadow sources present in git | 172 |
| Entries licensing non-text sections | 28 |
| Entries actually production-enrolled at `promoted_source` | 141 |
| Existing promotion paths that disagree with delinks | 0 |

Manifest statuses: `link-verified` 3, `promoted` 136, `text-verified` 33.
