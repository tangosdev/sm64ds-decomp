...
| Strict CONVERTED source functions | 2702 / 11358 (23.79%) |
| Physical production source files | 9168 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 9168 |
| Tracked `.c` files | 5264 |
| Tracked `.cpp` files | 3904 |
| `.cpp` files missing first-line `//cpp` | 0 |
| Mangled-symbol source files | 3991 |
| Genuinely migrated C++ symbol files | 3911 |
...
| Source-built (`complete`) source files | 9002 |
| Complete sources owning more than one function | 131 |
| Source-built records inside multi-function sources | 2326 |
...
| Functions named by manifest entries | 2952 |
| Unique functions named by the manifest | 2941 |
| Modules represented | 47 |
| Manifest shadow sources present in git | 160 |
| Entries licensing non-text sections | 20 |
| Entries actually production-enrolled at `promoted_source` | 130 |
| Existing promotion paths that disagree with delinks | 0 |

Manifest statuses: `link-verified` 3, `promoted` 130, `text-verified` 27.
