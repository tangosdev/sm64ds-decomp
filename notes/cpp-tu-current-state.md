...
| Physical production source files | 10454 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 10454 |
| Tracked `.c` files | 6006 |
| Tracked `.cpp` files | 4448 |
...
| Source-built (`complete`) source files | 10230 |
| Complete sources owning more than one function | 92 |
| Source-built records inside multi-function sources | 960 |
...
| Entries actually production-enrolled at `promoted_source` | 91 |
| Existing promotion paths that disagree with delinks | 0 |

Manifest statuses: `link-verified` 4, `promoted` 91, `shadow` 1, `text-verified` 28.
