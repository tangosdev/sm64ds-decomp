...
| Strict CONVERTED source functions | 2571 / 11312 (22.73%) |
| Physical production source files | 11250 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 11250 |
| Tracked `.c` files | 6375 |
| Tracked `.cpp` files | 4875 |
...
| Source-built (`complete`) source files | 11024 |
| Complete sources owning more than one function | 7 |
| Source-built records inside multi-function sources | 74 |
...
Current complete multi-function owners: `src/actors/ActorBase_SceneNode.cpp` (2 function-symbol records), `src/actors/daKpFr_c.cpp` (21 function-symbol records), `src/actors/daKrpa_c.cpp` (25 function-symbol records), `src/actors/daObjAbuku_c.cpp` (7 function-symbol records), `src/actors/daObjKm3_Kurumajiku_c.cpp` (5 function-symbol records), `src/actors/daObjPathLift_c.cpp` (8 function-symbol records), `src/func_01ff97d8.c` (6 function-symbol records).
...
| Measure | Live value |
| --- | ---: |
| Tracked `src_tu/` source files | 83 |
| Manifest entries | 89 |
...
| Entries actually production-enrolled at `promoted_source` | 6 |
| Existing promotion paths that disagree with delinks | 0 |

Manifest statuses: `link-verified` 5, `promoted` 6, `text-verified` 78.
