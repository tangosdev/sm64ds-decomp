...
| Physical production source files | 10934 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 10934 |
| Tracked `.c` files | 6187 |
| Tracked `.cpp` files | 4747 |
...
| Source-built (`complete`) source files | 10707 |
| Complete sources owning more than one function | 44 |
| Source-built records inside multi-function sources | 429 |
...
| Measure | Live value |
| --- | ---: |
| Tracked `src_tu/` source files | 55 |
| Manifest entries | 98 |
...
| Entries actually production-enrolled at `promoted_source` | 39 |
| Existing promotion paths that disagree with delinks | 4 |

Manifest statuses: `link-verified` 3, `promoted` 43, `text-verified` 52.
