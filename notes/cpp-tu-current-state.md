...
| Physical production source files | 11012 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 11012 |
| Tracked `.c` files | 6262 |
| Tracked `.cpp` files | 4750 |
| `.cpp` files missing first-line `//cpp` | 0 |
| Mangled-symbol source files | 3633 |
| Genuinely migrated C++ symbol files | 3513 |
| Not semantically migrated | 117 |
...
| Source-built (`complete`) source files | 10785 |
| Complete sources owning more than one function | 31 |
| Source-built records inside multi-function sources | 338 |
...
| Measure | Live value |
| --- | ---: |
| Tracked `src_tu/` source files | 67 |
| Manifest entries | 97 |
| Functions named by manifest entries | 1245 |
| Unique functions named by the manifest | 1245 |
| Modules represented | 28 |
| Manifest shadow sources present in git | 97 |
| Entries licensing non-text sections | 4 |
| Entries actually production-enrolled at `promoted_source` | 26 |
| Existing promotion paths that disagree with delinks | 4 |

Manifest statuses: `link-verified` 3, `promoted` 30, `text-verified` 64.
