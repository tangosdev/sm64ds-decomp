...
| Physical production source files | 10948 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 10948 |
| Tracked `.c` files | 6190 |
| Tracked `.cpp` files | 4758 |
...
| Source-built (`complete`) source files | 10721 |
| Complete sources owning more than one function | 42 |
| Source-built records inside multi-function sources | 413 |
...
| Measure | Live value |
| --- | ---: |
| Tracked `src_tu/` source files | 57 |
| Manifest entries | 98 |
...
| Entries actually production-enrolled at `promoted_source` | 37 |
| Existing promotion paths that disagree with delinks | 4 |

Manifest statuses: `link-verified` 3, `promoted` 41, `text-verified` 54.
