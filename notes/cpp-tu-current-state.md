...
| Physical production source files | 9188 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 9188 |
| Tracked `.c` files | 5274 |
| Tracked `.cpp` files | 3914 |
...
| Source-built (`complete`) source files | 9022 |
| Complete sources owning more than one function | 132 |
| Source-built records inside multi-function sources | 2307 |
...
| Functions named by manifest entries | 2943 |
| Unique functions named by the manifest | 2932 |
| Modules represented | 46 |
| Manifest shadow sources present in git | 162 |
| Entries licensing non-text sections | 23 |
| Entries actually production-enrolled at `promoted_source` | 131 |
| Existing promotion paths that disagree with delinks | 0 |

Manifest statuses: `link-verified` 3, `promoted` 131, `text-verified` 28.
