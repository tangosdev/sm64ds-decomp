...
| Physical production source files | 9603 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 9603 |
| Tracked `.c` files | 5528 |
| Tracked `.cpp` files | 4075 |
...
| Path-owned records still supplied from ROM bytes | 111 |
| Source-built (`complete`) function-symbol records | 11146 |
| Source-built (`complete`) source files | 9399 |
| Complete sources owning more than one function | 117 |
| Source-built records inside multi-function sources | 1864 |
...
| Functions named by manifest entries | 2522 |
| Unique functions named by the manifest | 2511 |
| Modules represented | 43 |
| Manifest shadow sources present in git | 147 |
| Entries licensing non-text sections | 17 |
| Entries actually production-enrolled at `promoted_source` | 116 |
| Existing promotion paths that disagree with delinks | 0 |

Manifest statuses: `link-verified` 3, `promoted` 116, `shadow` 1, `text-verified` 27.
