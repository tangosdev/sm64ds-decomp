...
| Physical production source files | 10925 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 10925 |
| Tracked `.c` files | 6184 |
| Tracked `.cpp` files | 4741 |
...
| Source-built (`complete`) source files | 10698 |
| Complete sources owning more than one function | 47 |
| Source-built records inside multi-function sources | 441 |
...
| Measure | Live value |
| --- | ---: |
| Tracked `src_tu/` source files | 52 |
| Manifest entries | 98 |
...
| Entries actually production-enrolled at `promoted_source` | 42 |
| Existing promotion paths that disagree with delinks | 4 |

Manifest statuses: `link-verified` 3, `promoted` 46, `text-verified` 49.
