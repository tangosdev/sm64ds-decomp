...
| Strict CONVERTED source functions | 2716 / 11357 (23.91%) |
| Physical production source files | 8975 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 8975 |
| Tracked `.c` files | 5172 |
| Tracked `.cpp` files | 3803 |
...
| Source-built (`complete`) source files | 8817 |
| Complete sources owning more than one function | 140 |
| Source-built records inside multi-function sources | 2527 |
...
| Manifest entries | 175 |
| Functions named by manifest entries | 3168 |
| Unique functions named by the manifest | 3157 |
| Modules represented | 49 |
| Manifest shadow sources present in git | 175 |
| Entries licensing non-text sections | 29 |
| Entries actually production-enrolled at `promoted_source` | 144 |
| Existing promotion paths that disagree with delinks | 0 |

Manifest statuses: `link-verified` 3, `promoted` 139, `text-verified` 33.
