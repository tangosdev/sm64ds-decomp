...
| Measure | Live value |
| --- | ---: |
| MATCHED functions | 11272 / 11402 (98.86%) |
| MATCHED code bytes | 2144856 / 2238108 (95.83%) |
| Strict CONVERTED source functions | 2671 / 11328 (23.58%) |
| Physical production source files | 10331 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 10331 |
| Tracked `.c` files | 5972 |
| Tracked `.cpp` files | 4359 |
| `.cpp` files missing first-line `//cpp` | 0 |
| Mangled-symbol source files | 3848 |
| Genuinely migrated C++ symbol files | 3722 |
| Not semantically migrated | 124 |
| `.cpp` files still hand-spelling their symbol | 57 |
| Nonmatching C++-symbol drafts | 9 |
| Delinks path-owned function-symbol records | 11235 |
| Path-owned records still supplied from ROM bytes | 123 |
| Source-built (`complete`) function-symbol records | 11112 |
| Source-built (`complete`) source files | 10110 |
| Complete sources owning more than one function | 103 |
| Source-built records inside multi-function sources | 1105 |
...
Current complete multi-function owners: `src/actors/ActorBase_SceneNode.cpp` (2 function-symbol records), `src/actors/ActorDerived.cpp` (5 function-symbol records), `src/actors/dBgActor_c.cpp` (11 function-symbol records), `src/actors/dScMgD3DBase_c.cpp` (27 function-symbol records), `src/actors/daDsnBase_c.cpp` (9 function-symbol records), `src/actors/daObjCtMecha03_c.cpp` (8 function-symbol records), `src/actors/daObjCtMecha04_c.cpp` (9 function-symbol records), `src/actors/daObjFallBlock_c.cpp` (10 function-symbol records), `src/actors/daObjFloatBoard_c.cpp` (5 function-symbol records), `src/actors/daOts_c.cpp` (22 function-symbol records), and 93 more in `--json`.
...
| Manifest entries | 134 |
| Functions named by manifest entries | 1816 |
| Unique functions named by the manifest | 1805 |
| Modules represented | 39 |
| Manifest shadow sources present in git | 134 |
| Entries licensing non-text sections | 16 |
| Entries actually production-enrolled at `promoted_source` | 102 |
| Existing promotion paths that disagree with delinks | 0 |

Manifest statuses: `link-verified` 4, `promoted` 102, `shadow` 1, `text-verified` 27.
