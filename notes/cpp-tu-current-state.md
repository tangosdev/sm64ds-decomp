...
| Strict CONVERTED source functions | 2599 / 11315 (22.97%) |
| Physical production source files | 10394 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 10394 |
| Tracked `.c` files | 6027 |
| Tracked `.cpp` files | 4367 |
| `.cpp` files missing first-line `//cpp` | 0 |
| Mangled-symbol source files | 3685 |
| Genuinely migrated C++ symbol files | 3603 |
...
| Path-owned records still supplied from ROM bytes | 120 |
| Source-built (`complete`) function-symbol records | 11094 |
| Source-built (`complete`) source files | 10168 |
| Complete sources owning more than one function | 114 |
| Source-built records inside multi-function sources | 1040 |
...
| Measure | Live value |
| --- | ---: |
| Tracked `src_tu/` source files | 26 |
| Manifest entries | 139 |
| Functions named by manifest entries | 1645 |
| Unique functions named by the manifest | 1645 |
| Modules represented | 43 |
| Manifest shadow sources present in git | 139 |
| Entries licensing non-text sections | 35 |
| Entries actually production-enrolled at `promoted_source` | 109 |
| Existing promotion paths that disagree with delinks | 4 |

Manifest statuses: `link-verified` 3, `promoted` 113, `shadow` 1, `text-verified` 22.
