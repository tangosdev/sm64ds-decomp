...
| Strict CONVERTED source functions | 2605 / 11315 (23.02%) |
| Physical production source files | 10498 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 10498 |
| Tracked `.c` files | 6036 |
| Tracked `.cpp` files | 4462 |
| `.cpp` files missing first-line `//cpp` | 0 |
| Mangled-symbol source files | 3696 |
| Genuinely migrated C++ symbol files | 3614 |
...
| Source-built (`complete`) source files | 10271 |
| Complete sources owning more than one function | 96 |
| Source-built records inside multi-function sources | 918 |
...
| Functions named by manifest entries | 1544 |
| Unique functions named by the manifest | 1544 |
| Modules represented | 38 |
| Manifest shadow sources present in git | 125 |
| Entries licensing non-text sections | 15 |
| Entries actually production-enrolled at `promoted_source` | 91 |
| Existing promotion paths that disagree with delinks | 4 |

Manifest statuses: `link-verified` 3, `promoted` 95, `text-verified` 27.
