...
| Physical production source files | 9234 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 9234 |
| Tracked `.c` files | 5288 |
| Tracked `.cpp` files | 3946 |
| `.cpp` files missing first-line `//cpp` | 0 |
| Mangled-symbol source files | 4002 |
| Genuinely migrated C++ symbol files | 3922 |
...
| Source-built (`complete`) source files | 9067 |
| Complete sources owning more than one function | 127 |
| Source-built records inside multi-function sources | 2256 |
...
| Functions named by manifest entries | 2882 |
| Unique functions named by the manifest | 2871 |
| Modules represented | 46 |
| Manifest shadow sources present in git | 156 |
| Entries licensing non-text sections | 17 |
| Entries actually production-enrolled at `promoted_source` | 126 |
| Existing promotion paths that disagree with delinks | 0 |

Manifest statuses: `link-verified` 3, `promoted` 126, `text-verified` 27.
