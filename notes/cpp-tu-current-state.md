...
| Physical production source files | 9203 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 9203 |
| Tracked `.c` files | 5278 |
| Tracked `.cpp` files | 3925 |
...
| Source-built (`complete`) source files | 9037 |
| Complete sources owning more than one function | 130 |
| Source-built records inside multi-function sources | 2290 |
...
| Functions named by manifest entries | 2916 |
| Unique functions named by the manifest | 2905 |
| Modules represented | 46 |
| Manifest shadow sources present in git | 159 |
| Entries licensing non-text sections | 20 |
| Entries actually production-enrolled at `promoted_source` | 129 |
| Existing promotion paths that disagree with delinks | 0 |

Manifest statuses: `link-verified` 3, `promoted` 129, `text-verified` 27.
