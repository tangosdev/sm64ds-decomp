...
| Physical production source files | 8952 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 8952 |
| Tracked `.c` files | 5160 |
| Tracked `.cpp` files | 3792 |
...
| Source-built (`complete`) source files | 8794 |
| Complete sources owning more than one function | 141 |
| Source-built records inside multi-function sources | 2551 |
...
| Manifest entries | 176 |
| Functions named by manifest entries | 3192 |
| Unique functions named by the manifest | 3181 |
| Modules represented | 50 |
| Manifest shadow sources present in git | 176 |
| Entries licensing non-text sections | 29 |
| Entries actually production-enrolled at `promoted_source` | 145 |
| Existing promotion paths that disagree with delinks | 0 |

Manifest statuses: `link-verified` 3, `promoted` 140, `text-verified` 33.
