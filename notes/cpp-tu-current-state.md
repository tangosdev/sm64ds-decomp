...
| Physical production source files | 10958 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 10958 |
| Tracked `.c` files | 6192 |
| Tracked `.cpp` files | 4766 |
...
| Source-built (`complete`) source files | 10731 |
| Complete sources owning more than one function | 40 |
| Source-built records inside multi-function sources | 401 |
...
| Entries actually production-enrolled at `promoted_source` | 35 |
| Existing promotion paths that disagree with delinks | 4 |

Manifest statuses: `link-verified` 3, `promoted` 39, `text-verified` 56.
