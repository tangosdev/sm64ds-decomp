...
| Physical production source files | 10962 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 10962 |
| Tracked `.c` files | 6193 |
| Tracked `.cpp` files | 4769 |
...
| Source-built (`complete`) source files | 10735 |
| Complete sources owning more than one function | 39 |
| Source-built records inside multi-function sources | 396 |
...
| Measure | Live value |
| --- | ---: |
| Tracked `src_tu/` source files | 60 |
| Manifest entries | 98 |
...
| Entries actually production-enrolled at `promoted_source` | 34 |
| Existing promotion paths that disagree with delinks | 4 |

Manifest statuses: `link-verified` 3, `promoted` 38, `text-verified` 57.
