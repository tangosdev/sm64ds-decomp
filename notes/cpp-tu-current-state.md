...
| Physical production source files | 10954 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 10954 |
| Tracked `.c` files | 6191 |
| Tracked `.cpp` files | 4763 |
...
| Source-built (`complete`) source files | 10727 |
| Complete sources owning more than one function | 41 |
| Source-built records inside multi-function sources | 406 |
...
| Measure | Live value |
| --- | ---: |
| Tracked `src_tu/` source files | 58 |
| Manifest entries | 98 |
...
| Entries actually production-enrolled at `promoted_source` | 36 |
| Existing promotion paths that disagree with delinks | 4 |

Manifest statuses: `link-verified` 3, `promoted` 40, `text-verified` 55.
