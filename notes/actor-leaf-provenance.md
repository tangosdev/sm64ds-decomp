...
Both `src/game/actors/Ukiki/_ZN5Ukiki13InitResourcesEv.cpp` and `src/game/actors/Ukiki/_ZN5Ukiki8BehaviorEv.cpp`
...
| offset | new name | evidence |
| --- | --- | --- |
| 0x138 | `mKeyModel` | `new Model` + `ModelBase::SetFile` in `src/game/actors/Door/_ZN4Door13InitResourcesEv.c`; `Virtual10(mModel.data.transforms)` then `Render(0)` in `src/game/actors/Door/_ZN4Door6RenderEv.cpp`, where the local holding it is already called `key`; `delete key` through Model's vtable slot 1 in `src/game/actors/Door/_ZN4Door16CleanupResourcesEv.cpp`. Owned by the Door. |
...
| 0x140 | `mCallbackNode` | `src/game/actors/Door/_ZN4Door8BehaviorEv.cpp` casts it to a node whose `+0x8` is a `void (Door::*)(int)` and calls it on this Door. |
...
the existing note in `src/game/actors/Door/_ZN4Door13InitResourcesEv.c` about that triple stands.
...
Sources: `src/game/actors/daSCoin_c/_ZN9daSCoin_c13InitResourcesEv.cpp`,
`src/game/actors/daSCoin_c/_ZN9daSCoin_c8BehaviorEv.cpp`.
...
(`actorID` 0xf1). `src/game/actors/Pokey/_ZN5Pokey13InitResourcesEv.cpp` branches on that all the
way through.
...
| 0x38c | `mState` | `src/game/actors/Pokey/_ZN5Pokey8BehaviorEv.cpp` skips its distance-to-player early-out when this is 2 or 5. |
...
| 0x394 | `mNextSegment` | `src/game/actors/Pokey/_ZN5Pokey16OnPendingDestroyEv.cpp` (head only) walks `p = mNextSegment` and then `p->mNextSegment` at the same 0x394 offset, tearing down each segment in turn. |
...
| offset | new name | evidence |
| --- | --- | --- |
| 0x350 | `mSpawnPosX` | `src/game/actors/BabyPenguin/_ZN11BabyPenguin13InitResourcesEv.cpp` copies `mPosX` in; never written again. |
| 0x354 | `mSpawnPosY` | same, `mPosY`. |
| 0x358 | `mSpawnPosZ` | same, `mPosZ`. |
| 0x364 | `mCachedActor` | `src/game/actors/BabyPenguin/_ZN11BabyPenguin8BehaviorEv.cpp`: `if (mCachedActor == 0) mCachedActor = FindWithActorID(0x101, 0)`, lazily filled and never cleared. A `dActor_c*` spelt `s32`. |
...
| offset | new name | evidence |
| --- | --- | --- |
| 0x3c0 | `mMatrix` | `*(Matrix4x3*)&unk_3c0 = IDENTITY_MATRIX4X3` in `src/game/actors/Lakitu/_ZN6Lakitu13InitResourcesEv.cpp`. The pad after it runs to 0x3f3, four bytes past the matrix's own 0x30. |
| 0x3f4 | `mState` | `src/game/actors/Lakitu/_ZN6Lakitu6RenderEv.cpp` draws the second `Model` only when this is 1. |
...
| 0x3d0 | `mSpawnPosX` | `src/game/actors/Moneybag/_ZN8Moneybag13InitResourcesEv.cpp` copies `mPosX` in. |
| 0x3d4 | `mSpawnPosY` | same, `mPosY`. |
| 0x3d8 | `mSpawnPosZ` | same, `mPosZ`. |
| 0x3f0 | `mState` | set to 1 by `InitResources`; `src/game/actors/Moneybag/_ZN8Moneybag6RenderEv.cpp` draws the `ModelAnim` only above 1 and the `Model` only at or below 0x1f. |
...
| 0x3ae | `mCoinFlags` | the flag byte `src/game/actors/Coin/_ZN4Coin13InitResourcesEv.cpp` already documented as such: bit 0 gates `Render` outright, `Behavior` tests bits 0 and 1. Named `mCoinFlags` and NOT `mFlags`, which is `dActor_c`'s own field 0x0b0. |
...
`src/game/actors/Coin/_ZN4Coin13InitResourcesEv.cpp` explains why.

## PowerFlower -- include/PowerFlower.h

| offset | new name | evidence |
| --- | --- | --- |
| 0x3bc | `mGroundY` | `src/game/actors/PowerFlower/_ZN11PowerFlower13InitResourcesEv.cpp` raycasts a `dBgCh_Gnd` from `mPos` with Y + 0x14000 and stores the hit height (`ray + 0x44`), falling back to the probe's own Y when `DetectClsn` finds nothing. |
| 0x3c0 | `mState` | `src/game/actors/PowerFlower/_ZN11PowerFlower6RenderEv.cpp` switches on it: 0 draws `mModel1`, 1 and 2 draw `mModel2`. |
...
| offset | new name | evidence |
| --- | --- | --- |
| 0x138 | `mOwnerUniqueID` | `src/game/actors/Number/_ZN6Number8BehaviorEv.cpp` resolves it through `dActor_c::FindWithID` and, when the actor still exists, takes the popup's draw position from that actor's own `mPos` triple. 0 means "not following anything" -- the `owner` argument of `SpawnNumber`. |
| 0x13c | `mStartPosX` | copied from `mPosX` in `src/game/actors/Number/_ZN6Number13InitResourcesEv.cpp`, alongside the already-named `mStartPosY` at 0x140. |
...
- `CrazedCrate` 0x374: zeroed in `src/game/actors/CrazedCrate/_ZN11CrazedCrate13InitResourcesEv.cpp`, never read.
- `MontyMole` 0x17c/0x180..0x184/0x188 and the 16 bytes from 0x16c: all written
  in `src/game/actors/MontyMole/_ZN9MontyMole13InitResourcesEv.cpp` -- 0x180 is `param1 & 0xf`, 0x182
...
- `BowserShockwaves` 0x214: zeroed in
  `src/game/actors/BowserShockwaves/_ZN16BowserShockwaves13InitResourcesEv.cpp`, never read.
...
Source: `src/game/actors/SpikeBomb/_ZN9SpikeBomb13InitResourcesEv.cpp`. Both branches of the header
carry the rename.
...
Sources: `src/game/actors/daEyBm_c/_ZN8daEyBm_c13InitResourcesEv.cpp`,
`src/game/actors/daEyBm_c/_ZN8daEyBm_c6RenderEv.cpp`, `src/game/actors/daEyBm_c/_ZN8daEyBm_c8BehaviorEv.cpp`.
...
Sources: `src/game/actors/daKpFr_c/_ZN8daKpFr_c13InitResourcesEv.cpp`,
`src/game/actors/daKpFr_c/_ZN8daKpFr_c6RenderEv.cpp`, `src_tu/actors/daKpFr_c.cpp`.
...
Sources: `src/game/actors/daKrpa_c/_ZN8daKrpa_c13InitResourcesEv.cpp`, `src_tu/actors/daKrpa_c.cpp`.
...
Sources: `src/game/actors/Spiny/_ZN5Spiny13InitResourcesEv.cpp`, `src/game/actors/Spiny/_ZN5Spiny6RenderEv.cpp`,
`src/game/actors/Spiny/_ZN5Spiny8BehaviorEv.cpp`, `src_tu/actors/Spiny.cpp`.
...
Sources: `src/game/actors/EnemySwitchTag/_ZN14EnemySwitchTag13InitResourcesEv.cpp`,
`src/game/actors/EnemySwitchTag/_ZN14EnemySwitchTag8BehaviorEv.cpp`,
...
Source: `src/game/actors/daDossyCap_c/_ZN12daDossyCap_c8BehaviorEv.cpp`.
...
Source: `src/game/actors/Scuttlebug/_ZN10Scuttlebug8BehaviorEv.cpp`.
...
Source: `src/game/actors/BowserTail/_ZN10BowserTail8BehaviorEv.cpp`.

## More leaves searched, nothing named

- `BobOmbBuddy` 0x198: zeroed in `src/game/actors/BobOmbBuddy/_ZN11BobOmbBuddy13InitResourcesEv.cpp`,
  never read.
...
- `SoundObject` 0x0e0: filled from `data_ov002_0210c08a + param1 * 0xc`, the
  ...
  itself says. [`src/game/actors/SoundObject/_ZN11SoundObject13InitResourcesEv.cpp`]
