...
// into this->fileID. On the game targets func_02018a24 is the ROM's own
// (src/func_02018a24.c), reading the table ov000's entry builds at boot (run
// linkfull, lane S4OV0); only the narrow harnesses still get it from the asset
// catalog below (build/assets/handles.tsv: handle -> file_id -> path,
// generated by tools/asset_catalog.py from the ROM). File bytes come from
...
/* ov0 handle -> FAT file id (data_ov000_020bd4b8 on the DS)
   ONLY FOR THE NARROW HARNESSES since run linkfull wave 31 (lane S4OV0). The
   three hosting targets (walk_window, walk_window_hires, smoke_player) compile
   this file with SM64DS_OV0_TABLE_ROM and link src/func_02018a24.c, which reads
   the table the ROM builds: data_0209d3b8 (hal/s4ov0_ov0table.cpp), filled at
   boot by ov000's entry (hal/boot_os.cpp, port_boot_rom_worker). The smoke_*
   harnesses run no boot and keep this catalog lookup. */
#ifndef SM64DS_OV0_TABLE_ROM
/* PORT_HOST_ABI: the harnesses build no ov0 table (data_0209d3b8), so they
   resolve handles through the asset catalog instead. */
...
#else
/* src/func_02018a24.c: `unsigned int func_02018a24(unsigned int i)`. The
   callers below take the low half, as the ROM's own callers do. */
unsigned int func_02018a24(unsigned int handle);
#endif /* SM64DS_OV0_TABLE_ROM */
