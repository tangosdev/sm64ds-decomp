       ...
       function" rather than reflowing three dozen lines nobody changed. */
    {
        ...
        if (tick_game) {
            ...
            /* THE FRAME CLOCK, func_020197b8 phase 6 (hal/fader_wipes.cpp). After
               the actor phases and before the render, which is where the ROM's
               own loop steps it. Every blink in the game hangs off it, and on
               this path that includes the only visual difference between a
               SELECTED Pair-a-Gone card and an idle one. */
