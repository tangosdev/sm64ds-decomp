 ...
 *
 * BOTH ARE THE ROM'S OWN CODE NOW. The second one, func_ov002_020f7d74, took
 * the same rung in run linkfull lane PMF2: see THE SECOND DISPATCHER'S RUNG 2
 * below.
 *
...
/* Called from ov002's bring-up in tests/walk_window.cpp and
   ...
   {its own address, 0}. */
static void port_kuppa_objcmd_seat(void);   /* the second dispatcher's, below */
...
/* ---- THE SECOND DISPATCHER'S RUNG 2 (run linkfull lane PMF2) --------------
 *
 * src/func_ov002_020f7d74.cpp is the ROM's own code now, on
 * port/slice_w27_pmf2.txt, and this file no longer defines that symbol. It is
 * the first dispatcher's shape exactly (RUNG 2 above): an incomplete class, the
 * same `void (C::*PMF)(unsigned char*, int, int)`, a function-local static
 * table copied out of the records on the first call, and
 * `(self->*table[p[6]])(p + 7, a2, a3)` -- receiver in ecx, three stack words,
 * callee-cleaned. So it takes the same seat, writing a __fastcall face into
 * each record before the first call, and the faces. It takes NO
 * /alternatename rows: this TU declares its twenty-four records inside its
 * own extern "C" block, and the object's symbol table asks for the plain C
 * names (_data_ov002_0210b708 and the rest), which the ov002 mount defines.
 *
 * THE UNIVERSE IS CLOSED BY THE ROM. The twenty-four records are {code, 0},
 * each word 0 carrying a `load` relocation to its own body and no relocation
 * leaving +4 (runs/linkfull/out/PMF2/F_romrec.txt), and the ONLY references to
 * any of the twenty-four record addresses in ov002 or main are this row's own
 * twenty-four literal-pool loads at 020f7fc4..020f8024 (F_allrefs.txt). No
 * sinit and no second reader can see them, so rewriting the mounted word
 * disturbs nothing else. The records are ov002 .data, a generated byte array in
 * the image like the first dispatcher's fourteen, and nothing refills them.
 *
 * THE SEAT RUNS FROM port_kuppa_cmd_seat, which the harnesses already call in
 * ov002's bring-up, so no call site changes: the first command of any script
 * comes long after it. It refuses, loudly, on any record that does not hold
 * the ROM's own {address, 0} first.
 *
 * THE SAME THING IS LOST as in rung 2, and it is recorded the same way: the
 * host copy refused a command index of 24 or more; the ROM indexes this table
 * with no bound and so does the matched TU.
 *
 * THE FUNCTION-LOCAL STATIC. MSVC guards the table's first-call initialiser
 * with its thread-safe statics (__Init_thread_header / _footer and the TLS
 * epoch). Those were already in the image before this row -- the first
 * dispatcher's table uses them -- so the TLS directory and the KERNEL32 import
 * set are measured unchanged, not assumed (the lane's final message carries
 * both before and after).
 */

/* The census the faces keep, dumped beside the first dispatcher's under
   SM64DS_KUPPA_LOG: the plain game's own count of every command it ran. */
static unsigned g_objcmd_hits[24];

#define KUPPA_OBJ_FACE(ix, sym)                                               \
    static void __fastcall objcmd_face_##sym(void *self, void *dead_edx,      \
                                             unsigned char *p, int a2, int a3)\
    {                                                                         \
        (void)dead_edx;                                                       \
        ++g_objcmd_hits[ix];                                                  \
...
KUPPA_OBJ_FACE(0,  func_ov002_020f7bb8)
KUPPA_OBJ_FACE(1,  func_ov002_020f79c0)
KUPPA_OBJ_FACE(2,  func_ov002_020f7780)
KUPPA_OBJ_FACE(3,  func_ov002_020f7538)
KUPPA_OBJ_FACE(4,  func_ov002_020f7410)
KUPPA_OBJ_FACE(5,  func_ov002_020f7384)
KUPPA_OBJ_FACE(6,  func_ov002_020f72bc)
KUPPA_OBJ_FACE(7,  func_ov002_020f71c4)
KUPPA_OBJ_FACE(8,  func_ov002_020f7038)
KUPPA_OBJ_FACE(9,  func_ov002_020f7020)
KUPPA_OBJ_FACE(10, func_ov002_020f6e48)
KUPPA_OBJ_FACE(11, func_ov002_020f6c60)
KUPPA_OBJ_FACE(12, func_ov002_020f6c34)
KUPPA_OBJ_FACE(13, func_ov002_020f6c24)
KUPPA_OBJ_FACE(14, func_ov002_020f6bc0)
KUPPA_OBJ_FACE(15, func_ov002_020f6b4c)
KUPPA_OBJ_FACE(16, func_ov002_020f6b28)
KUPPA_OBJ_FACE(17, func_ov002_020f6ae4)
KUPPA_OBJ_FACE(18, func_ov002_020f6ab8)
KUPPA_OBJ_FACE(19, func_ov002_020f6a9c)
KUPPA_OBJ_FACE(20, func_ov002_020f6448)
KUPPA_OBJ_FACE(21, func_ov002_020f63f8)
KUPPA_OBJ_FACE(22, func_ov002_020f26d4)
KUPPA_OBJ_FACE(23, func_ov002_020f26c4)

/* command index -> {the ROM slot, the address the record must hold, the face,
   its name}. The slot order is the ROM's own initialiser order, which is the
   command index order. port/tools/pmf_guard.py checks every face word of this
   table for an ECX receiver. */
typedef void (__fastcall *KuppaObjFace)(void *, void *, unsigned char *, int, int);
static const struct {
    PortPmf *slot;
    unsigned rom;
    KuppaObjFace face;
    ...
    { data_ov002_0210b708, 0x020f7bb8, objcmd_face_func_ov002_020f7bb8, "0210b708" },
    { data_ov002_0210b678, 0x020f79c0, objcmd_face_func_ov002_020f79c0, "0210b678" },
    { data_ov002_0210b618, 0x020f7780, objcmd_face_func_ov002_020f7780, "0210b618" },
    { data_ov002_0210b620, 0x020f7538, objcmd_face_func_ov002_020f7538, "0210b620" },
    { data_ov002_0210b640, 0x020f7410, objcmd_face_func_ov002_020f7410, "0210b640" },
    { data_ov002_0210b628, 0x020f7384, objcmd_face_func_ov002_020f7384, "0210b628" },
    { data_ov002_0210b648, 0x020f72bc, objcmd_face_func_ov002_020f72bc, "0210b648" },
    { data_ov002_0210b8c0, 0x020f71c4, objcmd_face_func_ov002_020f71c4, "0210b8c0" },
    { data_ov002_0210b900, 0x020f7038, objcmd_face_func_ov002_020f7038, "0210b900" },
    { data_ov002_0210b938, 0x020f7020, objcmd_face_func_ov002_020f7020, "0210b938" },
    { data_ov002_0210b930, 0x020f6e48, objcmd_face_func_ov002_020f6e48, "0210b930" },
    { data_ov002_0210b928, 0x020f6c60, objcmd_face_func_ov002_020f6c60, "0210b928" },
    { data_ov002_0210b920, 0x020f6c34, objcmd_face_func_ov002_020f6c34, "0210b920" },
    { data_ov002_0210b918, 0x020f6c24, objcmd_face_func_ov002_020f6c24, "0210b918" },
    { data_ov002_0210b910, 0x020f6bc0, objcmd_face_func_ov002_020f6bc0, "0210b910" },
    { data_ov002_0210b908, 0x020f6b4c, objcmd_face_func_ov002_020f6b4c, "0210b908" },
    { data_ov002_0210b750, 0x020f6b28, objcmd_face_func_ov002_020f6b28, "0210b750" },
    { data_ov002_0210b8f8, 0x020f6ae4, objcmd_face_func_ov002_020f6ae4, "0210b8f8" },
    { data_ov002_0210b8f0, 0x020f6ab8, objcmd_face_func_ov002_020f6ab8, "0210b8f0" },
    { data_ov002_0210b8e8, 0x020f6a9c, objcmd_face_func_ov002_020f6a9c, "0210b8e8" },
    { data_ov002_0210b8e0, 0x020f6448, objcmd_face_func_ov002_020f6448, "0210b8e0" },
    { data_ov002_0210b8d8, 0x020f63f8, objcmd_face_func_ov002_020f63f8, "0210b8d8" },
    { data_ov002_0210b8d0, 0x020f26d4, objcmd_face_func_ov002_020f26d4, "0210b8d0" },
    { data_ov002_0210b8c8, 0x020f26c4, objcmd_face_func_ov002_020f26c4, "0210b8c8" },
};

static void kuppa_objcmd_census_dump(void)
{
    const char *path = std::getenv("SM64DS_KUPPA_LOG");
    const char *lvl = std::getenv("SM64DS_LEVEL");
    const char *scn = std::getenv("SM64DS_SCENE");
    ...
    for (unsigned i = 0; i < 24; ++i)
        total += g_objcmd_hits[i];
    std::fprintf(f, "kuppaobj level=%s scene=%s total=%u", lvl ? lvl : "-",
                 scn ? scn : "-", total);
    for (unsigned i = 0; i < 24; ++i)
        std::fprintf(f, " %s=%u", g_objcmd_seats[i].name, g_objcmd_hits[i]);
    std::fputc('\n', f);
    std::fclose(f);
}

/* Called from port_kuppa_cmd_seat above, once. */
static void port_kuppa_objcmd_seat(void)
{
    for (unsigned i = 0; i < 24; ++i) {
        PortPmf *slot = g_objcmd_seats[i].slot;
        if (slot->fn != g_objcmd_seats[i].rom || slot->delta != 0) {
            std::fprintf(stderr, "FATAL: cutscene-object command %u (%s): the "
                         "ov002 mount holds %08x/%d, the ROM's own record says "
                         "%08x/0 -- WRONG BYTES\n",
                         i, g_objcmd_seats[i].name, slot->fn, slot->delta,
                         g_objcmd_seats[i].rom);
            std::abort();
        }
        slot->fn = (unsigned)(size_t)g_objcmd_seats[i].face;
    }
    std::atexit(kuppa_objcmd_census_dump);
    if (std::getenv("SM64DS_KUPPA_SEAT_REPORT"))
        std::printf("[kuppa] twenty-four cutscene-object command records "
                    "seated, index 0 %08x .. index 23 %08x\n",
                    g_objcmd_seats[0].slot->fn, g_objcmd_seats[23].slot->fn);
}
