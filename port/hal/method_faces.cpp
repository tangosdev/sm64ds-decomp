...
/* ---- wave 4 lane d: the two FaderBrightness faces Scene::BeforeBehavior needs
   ...
   THE NULL IS GONE, run link100 lane CTOR2 rung C1c. This paragraph used to
   end "Do not wake the branch without giving data_0209f5d0 a real vptr first",
   because the only receiver that reaches SetForwardTime here is
   &data_0209f5d0 and its host storage was zeroed BSS -- the port did not link
   the ROM's static initialiser for it. It links it now:
   src/__sinit_02074edc.c is on port/slice_ctor.txt and the ROM's own
   .ctor walk runs it at Entry, so the object is constructed the way the DS
   ...
   is a trap report rather than a crash, which is the whole of what C1c changed
   here. */
