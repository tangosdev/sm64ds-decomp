/* HOST MIRROR of src/_ZN6FlyGuy13InitResourcesEv.cpp with EIGHT LINES MOVED
 * and nothing else changed: the eight `extern SharedFilePtr data_ov070_...`
 * / `extern PMF data_ov070_0212359c` declarations sit OUTSIDE the matched
 * TU's own `extern "C"` block, so MSVC gives them C++ linkage and the TU
 * asks the linker for ?data_ov070_02123530@@3USharedFilePtr@@A and seven
 * siblings. The mount emits those symbols with C linkage, as every other
 * consumer in the port spells them, so all eight come out as LNK2019.
 *
 * Here the same eight declarations are inside the extern "C" block. The body
 * below is character for character the matched TU's -- diff against it to
 * verify. This is the Coffin_InitResources.cpp shape (a host mirror whose
 * only delta is a declaration-linkage fix), not a reimplementation, and the
 * matched TU stays byte-locked in src/ as the proof, dropped from
 * slice_w5c.txt.
 *
 * mwcc does not care because the ROM's own C++ mangling is what the port
 * carries these names under anyway; this is purely an MSVC linkage-spelling
 * question at the declaration site.
 */
//cpp
