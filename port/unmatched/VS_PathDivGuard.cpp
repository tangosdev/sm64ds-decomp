/* PathLift's path-follow body (func_ov002_020ef670), host copy with the ARM
 * divider's semantics -- the VS wiring lane's substitution for the matched
 * TU in slice_w6d (the CMake loop skips the src line; slice_w6d.txt itself
 * is untouched).
 *
 * WHY: dv = cstd::fdiv(len2, speed) / 0x1000 is ZERO whenever a path
 * segment is shorter than one frame of travel, and the two `x / dv` sites
 * below then divide by zero. On the ARM the ROM's own divider answers
 * x / 0 = 0 (the AEABI idiv the ITCM block ships is `b ? a / b : 0` in the
 * port's own host body), so the DS glides through this and x86 hardware
 * idiv faults c0000094. Measured on the VS castle grounds (level 51, its
 * PathLift, actor id 0x1f) and reproduced with plain SM64DS_LEVEL=51 -- a
 * latent port-wide shape this map is simply the first to park a
 * slow-short-segment lift on. The copy changes the two division sites and
 * nothing else. PORT_HOST_ABI: ARM AEABI division semantics (x / 0 = 0).
 */
...
/* every helper this body calls is declared by decl_common.h / decl_PathPtr.h
   with C linkage already; only the TU's own bare externs need the wrapper */
extern "C" {
...
/* the ARM divider: x / 0 answers 0 (see the banner) */
static int arm_sdiv(int a, int b) { return b ? a / b : 0; }
...
extern "C" void func_ov002_020ef670(char *c)
{
    ...
    q2 = (short)arm_sdiv(ad2, dv);
    _Z14ApproachLinearRsss((short *)(c + 0x8e), sl, (short)arm_sdiv(ad1, dv));
