//cpp
/* ==========================================================================
 * PORT_HOST_ABI HOST COPY of src/actors/dScMgHanachan_c.cpp.
 * Run link100, wave 10 round 4, lane SCENES3, 2026-09-17.  Scene 386, the
 * "Which Wiggler" minigame (dScMgHanachan_c, actor id 0x182).
 * ==========================================================================
 *
 * THE DEFECT: ARGUMENT EVALUATION ORDER, in func_ov006_020eac38 (ROM ordinal
 * 12, 0x020eac38, size 0x3e0).  The matched source assigns the local t.urot
 * inside the comma expression that is ARGUMENT TWO of an OAM::Render call and
 * reads t.urot as ARGUMENT EIGHT of the same call.  C and C++ leave the order
 * of argument evaluation unspecified; mwccarm and MSVC pick opposite ends.
 *
 * THE FAULT IT CAUSED, headless, 600 frames, SM64DS_FAULTS_FATAL=1:
 *   c0000005 at _ZN3OAM6RenderEbP7OamAttriiii5Fix12IiEi+0x1d6, a READ of a
 *   DIFFERENT junk address on every run (0xf62e441e, 0xef703c9e, 0x1194e01a on
 *   three measurements).  The instruction is 0x0052a0a6
 *   `movsx eax, word ptr [edx*4 + 0x891b22]`, the sine table lookup, with edx
 *   the rotation angle after `sar edx, 4`; the angle is argument eight, read at
 *   OAM::Render's entry by 0x00529fd7 `mov edx, dword ptr [ebp + 0x24]`.
 *   Chain, walked with cdb rather than read off crash.txt (whose stack listing
 *   is guesswork): OAM::Render+0x1d6 <- func_ov006_020eac38+0x220 <-
 *   dScMgHanachan_c::Render+0x5b.
 *
 * THE CARTRIDGE, re-derived here with capstone over extracted/overlays/
 * overlay_0006.bin at ov006's load base 0x020bfec0 (the extracted overlay,
 * never the dsd copy; the base is self-checking, since the three calls below
 * land on the three ROM addresses the source names).  The call is at
 * 0x020eae10 and argument eight's outgoing slot is [sp,#0xc]:
 *
 *     020eadb4  bl   #0x20ebb40      t.res = func_ov006_020ebb40(o, i)
 *     020eadd8  bl   #0x20ebc08      a2i  = func_ov006_020ebc08(o, i)
 *     020eadec  lsl  r1, r8, #0x10
 *     020eadf0  lsr  r1, r1, #0x10   (unsigned short)rot, r8 = rot
 *     020eadf8  str  r1, [sp, #0x1c] t.urot = that
 *     020eadfc  str  r1, [sp, #0xc]  ARGUMENT EIGHT = THE SAME REGISTER
 *     020eae08  ldr  r1, [r1, r0, lsl #2]   argument two, the table index a2i
 *     020eae10  bl   #0x2021024      OAM::Render
 *
 * So on the cartridge argument eight is the value the store just wrote, and
 * the index call happens before the store.  The decomp's spelling is faithful.
 *
 * THE HOST, disassembled here out of this tree's own walk_window.exe at the
 * same call site (func_ov006_020eac38 = 0x005cbe40 in the map; t.urot lives at
 * [ebp-0x4c]):
 *
 *     005cbffa  mov  eax, dword ptr [ebp - 0x4c]    a COPY of t.urot, early
 *     005cc000  mov  dword ptr [ebp - 0x48], eax
 *     005cc03d  push dword ptr [ebp - 0x48]         ARGUMENT EIGHT, PUSHED
 *     005cc040  movzx esi, bx                       (unsigned short)rot
 *     005cc04f  mov  dword ptr [ebp - 0x4c], esi    t.urot = that, AFTER it
 *     005cc052  push dword ptr [eax*4 + 0x183d674]  argument two
 *     005cc05b  call 0x529ed0                       OAM::Render
 *
 * MSVC pushes a cdecl call's arguments right to left, so argument eight is
 * read before argument two assigns it.  On the first iteration of the i loop
 * that slot has never been written, which is why the faulting address differs
 * on every run and why the row passed in a window (where the stale word
 * happened to be small) and failed headless.
 *
 * WHAT THIS FILE IS.  It is src/actors/dScMgHanachan_c.cpp with ONE hunk
 * changed and nothing else, reproducible from the matched source:
 *
 *     python - <<'EOF'
 *     p = "src/actors/dScMgHanachan_c.cpp"
 *     t = open(p).read()
 *     t = t.replace(
 *       "        _ZN3OAM6RenderEbP7OamAttriiii5Fix12IiEi(\n"
 *       "            1,\n"
 *       "            data_ov006_02142018[(a2i = func_ov006_020ebc08(o, i), "
 *       "(t.urot = (unsigned short)rot), a2i)],\n",
 *       "        a2i = func_ov006_020ebc08(o, i);\n"
 *       "        t.urot = (unsigned short)rot;\n"
 *       "        _ZN3OAM6RenderEbP7OamAttriiii5Fix12IiEi(\n"
 *       "            1,\n"
 *       "            data_ov006_02142018[a2i],\n")
 *     open("port/unmatched/dScMgHanachan_c.cpp","w").write(t)
 *     EOF
 *
 * (plus this header).  src/ is NOT edited: the matched TU still byte-verifies
 * and mwccarm still sees exactly what the ROM was built from.
 *
 * WHY THE FILE KEEPS THE STEM dScMgHanachan_c.  This TU is on
 * PORT_HOSTGEN_TU in port/CMakeLists.txt and carries FIVE live hostgen patch
 * rows (four PMF_SEAM member-pointer dispatches and one MG_PMF_CALL that open-
 * codes the same decode in ints).  tools/hostgen.py keys every one of its patch
 * tables on the file STEM (emit(): `sym = src_path.stem`) and resolves any
 * repo-relative path (main(): `cand = src.parent / name`), so a copy under this
 * name goes on receiving all five rows verbatim while a copy under a new name
 * would silently drop them and put five DS code words back in the link.  The
 * PORT_HOSTGEN_TU row therefore names this file and port/slice_wig.txt names it
 * in the src TU's place; the generated copy under build/port/host-src/port/
 * unmatched/ is what compiles, exactly as before.
 *
 * WHAT IT COSTS THE LEDGER, stated rather than left to be noticed: nothing.
 * The object is still dScMgHanachan_c.cpp.obj, the same name the generated copy
 * of the src TU carried, so port/tools/linkage.py files its symbols exactly
 * where it filed them before and the headline does not move in either
 * direction.  This is a substitution of one generated copy for another, not a
 * new seat, and it raises no count.
 *
 * The SECOND OAM::Render call in the same function (ROM 0x020eaed8) also passes
 * t.urot, and there the cartridge loads it from the local it had already
 * stored; it is below the first call, so sequencing the store above the first
 * call leaves that site correct too.  One hunk covers both.
 * ==========================================================================
 */
...
void func_ov006_020eac38(void* o_)
{
    ...
    for (; i < 5; i++) {
        ...
        px = s.pos.y >> 12;
        py = s.pos.x >> 12;
        /* PORT_HOST_ABI: argument evaluation order.  The matched src spells
           the t.urot store inside the comma expression that is ARGUMENT TWO of
           this call and reads t.urot as ARGUMENT EIGHT of it.  The cartridge
           calls func_ov006_020ebc08 first, then stores the halfword, then
           writes the SAME register into argument eight's outgoing slot
           (0x020eadd8 / 0x020eadf8 / 0x020eadfc; the file header carries the
           disassembly).  MSVC pushes a cdecl call's arguments right to left, so
           it pushes argument eight from a copy taken before the store and the
           first loop iteration passes uninitialised stack.  The three
           statements below are the cartridge's own order, with the comma
           operator's sequence point replaced by two statement boundaries that
           no compiler may reorder. */
        a2i = func_ov006_020ebc08(o, i);
        t.urot = (unsigned short)rot;
        _ZN3OAM6RenderEbP7OamAttriiii5Fix12IiEi(
            1,
