...
struct ModelAnimVoidPtr;
struct ModelAnim { void SetAnim(void *f, int flags, int speed, unsigned start); };
...
static unsigned g_tte_trap_020cf2fc, g_tte_trap_020d01e0, g_tte_trap_020d0c38;

extern "C" void func_ov006_020cf2fc(char *)
{
    if (!g_tte_trap_020cf2fc)
        std::fprintf(stderr, "  [mg385] FLOOR func_ov006_020cf2fc (0x45c, no "
                     "src, no delinks block) wanted\n");
    ++g_tte_trap_020cf2fc;
}

extern "C" void func_ov006_020d01e0(short *, short *, short *)
{
    if (!g_tte_trap_020d01e0)
        std::fprintf(stderr, "  [mg385] FLOOR func_ov006_020d01e0 (0x800, no "
                     "src, no delinks block) wanted\n");
    ++g_tte_trap_020d01e0;
}

extern "C" int func_ov006_020d0c38(unsigned short *, unsigned short *)
{
    if (!g_tte_trap_020d0c38)
        std::fprintf(stderr, "  [mg385] FLOOR func_ov006_020d0c38 (0x3ac, no "
                     "src, no delinks block) wanted -- the stroke-connected "
...
extern "C" void port_mg_tte_trap_counts(unsigned *a, unsigned *b, unsigned *c,
                                        unsigned *d)
{
    if (a) *a = g_tte_trap_020cf2fc;
    if (b) *b = g_tte_trap_020d01e0;
    if (c) *c = g_tte_trap_020d0c38;
