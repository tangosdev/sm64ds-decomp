...
Every fixture path below is a plain relative name (never a drive letter or
an absolute path): parse_cl_transcript never touches disk, it only reads
Path.name and dict-keys the Path objects, so a fixture only has to look
like what cl would echo and put in an error line, not point anywhere real.
...
def _files(*names):
    """Path objects for a batch, rooted under a fake relative tree (never
    touches disk; parse_cl_transcript only ever reads .name and dict-keys
    these objects)."""
    return [Path(f"src/{n}") for n in names]
...
def test_error_in_the_source_itself():
    ...
    transcript = (
        "foo.cpp\n"
        "src\\foo.cpp(10): error C2065: 'bar': undeclared identifier\n"
    )
...
def test_header_error_lands_on_second_of_three_sources():
    ...
    transcript = (
        "a.cpp\n"
        "b.cpp\n"
        "include\\foo.h(42): error C2065: 'baz': undeclared identifier\n"
        "c.cpp\n"
    )
    results = parse_cl_transcript(transcript, files)
    assert results[a] is None, "a.cpp compiled clean and must stay clean"
    assert results[b] == ("C2065", "include\\foo.h"), \
...
def test_same_basename_different_directories():
    ...
    first = Path("src/foo/util.cpp")
    second = Path("src/bar/util.cpp")
    files = [first, second]
    transcript = (
        "util.cpp\n"
        "util.cpp\n"
        "src\\bar\\util.cpp(5): error C2059: syntax error\n"
    )
    results = parse_cl_transcript(transcript, files)
...
def test_first_error_per_source_rule_still_holds():
    ...
    transcript = (
        "d.cpp\n"
        "src\\d.cpp(3): error C2065: 'x': undeclared identifier\n"
        "include\\bar.h(9): error C2143: syntax error\n"
    )
    results = parse_cl_transcript(transcript, files)
    assert results[files[0]] == ("C2065", None)
