...
ends in an exit code, never in a hang.
  ...
  R3  MODEL ON, ROM BRING-UP OFF (SM64DS_IPC_ROM_BOOT=0). The model is up and
      the ARM7 has claimed its channels, but no ROM PXI body is called. The
      game must still run --frames frames and exit clean, which is what says
      the model does not perturb the build it was added to.
...
def rung1(out_dir, frames):
    ...
    check("R1", "[arm7] claimed 6 channels" in txt,
...
def rung3(out_dir, frames):
    print("R3  model ON, ROM bring-up OFF: the model perturbs nothing")
    ...
    check("R3", fm is not None and int(fm.group(1)) >= frames,
          "the game ran %s frames with no ROM PXI call at all"
