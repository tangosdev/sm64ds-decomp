 ...
 * for its other ten children. So only the tail is declared (unk_034[6]) and
 ...
 * unk_034[6]. That spelling is not a stylistic pick: it is the only one of
...
struct cMgSmartball_board_c : cMgSmartball_object_c {
    ...
    u8  unk_034[6]; /* 0x034-0x039 -- the TAIL of a 9-element per-cell claimed
                        flag array that BEGINS AT 0x31, inside the base. Never
                        indexed through this name: the whole array is reached
                        as (&unk_031)[i], because a member cannot straddle the
                        base boundary. Declared only so the layout below sits
    ...
    s32 unk_03c[9]; /* 0x03c -- per-cell flip angle. SaveSnapshot advances a
    ...
    u8  unk_060[9]; /* 0x060 -- per-cell highlight flag, set by Update for
                        every cell on a line that is currently scoring. */
    ...
    s32 unk_06c[8]; /* 0x06c -- per-line countdown; SaveSnapshot ages each one
                        while positive, Update draws the line's banner while
                        it is positive. Eight lines, see above. */
    u8  unk_08c[8]; /* 0x08c -- per-line flag; Update draws the banner for a
                        line whose timer has expired when this is set and the
                        manager's own 0x4660 flag is up. */
    s32 unk_094;    /* 0x094 -- countdown; SaveSnapshot spends it and replays
                        a jingle once a second (% 60) while it lasts. Update
                        also reads bits 0x3c of it to blink the highlight. */
    s32 unk_098;    /* 0x098 -- selects which jingle unk_094 replays; four
                        cases, so a scored-line count rather than a timer. */
};
