...
/* daDsn_c -- the Thwomp (DOSUN, actor 161). `dsn` is dossun, its Japanese name.
 *
 * THE NAME IS THE CARTRIDGE'S, NOT COINED. The tree called this class `Thwomp`
 * until the rename that preceded its TU promotion. Measured from
 * extracted/overlays/overlay_0091.bin (module base 0x02130f00): the vtable
 * address point 0x02135174 carries its typeinfo pointer at 0x02135170, whose
 * value 0x0213512c is _ZTI7daDsn_c; that record's words are
 * { 0x0209a764 (__si_class_type_info vtable), 0x02135120 (name), 0x021137f8
 * (_ZTI11daDsnBase_c) }, and the name string at 0x02135120 reads literally
 * "7daDsn_c". No `Thwomp` RTTI record exists anywhere in the image.
 *
 ...
 *   _ZTV7daDsn_c  ov091 0x02135174  (its record sits at V-4)
 ...
 * either, it inherits both. The tree spelled the symbols `_ZN6Thwomp*`, which is
 ...
 *
 * SIZE 0x3a4, from daDsn_c_classInit's literal 932.
 *
...
typedef char daDsn_c_size_must_be_0x3a4[sizeof(daDsn_c) == 0x3a4 ? 1 : -1];
#endif
