...
/* Puzzle Panic. A square grid of panels, each showing one of two faces.
 * The deal flips mFlips of them; touch a panel to flip it and its
 * neighbours back. mLives starts at 3.
 */
struct dScMgPanel_c : dScMgBase_c {
    ...
    u8  mFace[0x24];         /* 0x4f1e face the deal started from */
    u8  mShown[0x24];        /* 0x4f42 face on screen, flipped by touches */
    u8  mSettled[0x24];      /* 0x4f66 1 once that panel's flip has finished */
    u8  pad_4f8a[0x24];
    u8  mFlipAt[0x30];       /* 0x4fae panels the deal flipped */
    u8  mFlipCount;          /* 0x4fde */
    u8  mClear;              /* 0x4fdf set once every panel is settled */
    u8  pad_4fe0[0x2];
    u8  mLives;              /* 0x4fe2 starts at 3; a miss takes one */
    u8  pad_4fe3[0x6];
    u8  mBusy;               /* 0x4fe9 a flip is still playing */
    u8  mMark;               /* 0x4fea cleared at the start of a round */
};
