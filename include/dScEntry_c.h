 ...
 *
 * FIELDS ABOVE 0x50 that source evidence has actually named so far --
 * everything else in the range is left as opaque storage, sized exactly by
 * the constructor/destructor evidence above, pending a members-focused pass:
...
struct dScEntry_c : dScene_c {
    u8 unk_050[0x233];    /* 0x050 -- opaque; see the derivation note above for
                              the two nested-object ranges this space holds
                              (icon_c[9] at 0x070, graphCallback_c[4] at 0x1b4) */
    u8 unk_283;            /* 0x283 */
    u8 unk_284;            /* 0x284 */
    u8 unk_285;            /* 0x285 */
    u8 pad_286;            /* 0x286 */
