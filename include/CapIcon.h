 ...
 *   dCapIcon_c::Unlink (0x020ab110) unlink, and reset every field
 *
 ...
 * is only consistent with +0x0c = prev and +0x10 = next. Unlink
...
struct dCapIcon_c {
    ...
    /* The actor wearing the cap, and a copy of that actor's uniqueID taken at
       link time. Unlink compares the owner against the live slot
    ...
    /* Which character's list this node is in -- the index into the list-head
       ...
       Unlink parks it at 3, so 3 is the "none" value for
       a three-entry table. */
    u8 mCharacter;                /* 0x18 */
    u8 unk_19;                    /* 0x19 */
    u8 unk_1a;                    /* 0x1a */
    /* Bit 0 is the cap bank, passed in by dCapEnemy_c::AddCap. Bit 1 is tested by
       ...
       Unlink sets it again on the way out, and returns
