 ...
 *
 *   0x320  s32   -- InitResources copies mPosY (dActor_c +0x60) here; Behavior
 *                    reads it back as a baseline added to a sine offset.
 *   0x324  s16   -- a per-tick step magnitude, +-(0x400/n); Behavior only.
 *   0x326  u16   -- a phase counter Behavior increments every call and compares
 *                    against several thresholds.
 *   0x328  s8    -- a stage counter, -1 sentinel, incremented/decremented and
 *                    read as `10 - value`.
 *   0x329  u8    -- toggled with XOR 1 in InitResources/Behavior; a flag.
 *   0x32a  --    -- never dereferenced by any function in the class; 2 bytes of
 *                    padding closing the s8/u8 pair up to the next s32.
 *   0x32c  u32   -- a Particle::System::New() handle, read back as that call's
 *                    own first argument (handle recycling).
 *   0x330  u32   -- a second, independent particle handle, same shape.
 *
 * 0x320 + 0x14 = 0x334, closing exactly on the factory's literal. Field NAMES
 * below are placeholders (unk_/pad_); types are proven from the accesses above,
 * not guessed.
 */
