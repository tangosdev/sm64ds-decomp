/* daKinopio_c, reconstructed from the ov085 RTTI record, vtable, factory,
 ...
 * 0x0212feb8 overrides only the usual fBase_c lifecycle slots. The word at
 * 0x0212feb4 loads that record, and the name string it points to at
 * 0x0212fe5c reads `11daKinopio_c`, so this spelling is the cartridge's own.
 * The tree previously coined this class `Toad`; metadata emitted by isolated
 * destructor objects is discard-only.
 *
 ...
 */
#ifndef DAKINOPIO_C_H
#define DAKINOPIO_C_H
...
struct daKinopio_c : dActor_c {
    typedef void (daKinopio_c::*StateFunc)();
...
typedef char daKinopio_c_State_size_must_be_0x14[
    sizeof(daKinopio_c::State) == 0x14 ? 1 : -1];
#endif
...
#ifndef SM64DS_PLATFORM_PC
/* ROM layout under mwccarm; host ABI divergence is tracked separately. */
typedef char daKinopio_c_size_must_be_0x210[
    sizeof(struct daKinopio_c) == 0x210 ? 1 : -1];
#endif
